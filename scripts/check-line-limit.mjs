...
//
// Ngoại lệ (không phải mã nguồn hoặc sẽ xoá): tài liệu *.md, lockfile, supabase/config.toml
// (CLI sinh), index.html gốc (bản cũ, xoá sau khi GitHub Pages chuyển sang build Vite).

import { execSync } from 'node:child_process';
import { readFileSync } from 'node:fs';

const LIMIT = 300;
const EXCLUDED = [/\.md$/, /package-lock\.json$/, /^supabase\/config\.toml$/, /^index\.html$/, /^mockup\//];

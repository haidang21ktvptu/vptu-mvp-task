> **Đã thay bằng `SPEC.md` phiên bản 3 (viết lại quanh phụ lục Công văn 1400-CV/VPTU, 16/9/2026). File này giữ để tra cứu mã yêu cầu cũ (AUTH-x, RLS-x, TASK-x, DIR-x, MSG-x, DASH-x) và ma trận policy đã áp; không cập nhật nữa.**
...
- `accounts`: bỏ `password` (0006), `assigned_domain` (0007); `id` = `auth.users.id` (FK `ON DELETE RESTRICT`, 0011 — tạo auth user trước rồi mới INSERT accounts); thêm `must_change_password boolean default true` (0004), `is_chief boolean default false` (0007, đúng 1 dòng true = Chánh Văn phòng), `is_system boolean default false` (0012, GĐ6 — tài khoản hệ thống `smoke_test` cho smoke test sau phát hành; frontend ẩn khỏi mọi danh sách cán bộ, RLS không đổi).
...
- **GĐ8 (0013, PR 8A-1)** — quản trị đặc quyền theo `docs/thiet-ke-theo-doi-kl-btvtu.md` Phần 5: `accounts.quan_tri_kl`, `accounts.quan_tri_he_thong` (chỉ GRANT SELECT cột; đổi chỉ qua `admin_dat_co()` / migration; cờ không kế thừa vai trò); `quyen_lich_su` (nhật ký cấp quyền, chỉ hàm ghi, chỉ `quan_tri_he_thong` đọc); `phu_trach_phong` (PCVP ↔ phòng, `tu_ngay`/`den_ngay`, EXCLUDE chống chồng kỳ, bảng tạo rỗng — chủ dự án nhập tay trên màn hình Quản trị) + `phu_trach(lanh_dao, phong, ngay)` + `admin_phan_cong_phong()`.
- **GĐ8 (0014–0016, PR 8A-2)** — module Theo dõi KL BTVTU (thiết kế Phần 2, 4, 6): danh mục `dm_nganh` (12), `dm_co_quan_trinh` (13), `dm_loai_thoi_han` (4), `dm_tien_do` (2) — `ten` nguyên văn sheet DanhMuc; `kl_cau_hinh` (ngưỡng sắp đến hạn 7, hạn phản hồi 2, ký ban hành 10, không cập nhật 30); `kl_hoi_nghi` (số HN + số KL + ngày ban hành ≤ hôm nay); `kl_nhiem_vu` (mã `NV-xxx`, chủ trì `NOT NULL` → `accounts`, ba mốc `han_xu_ly` / `ngay_hoan_thanh` / `cap_nhat_luc` + `ghi_hoan_thanh_luc`, `ly_do_chua_co_han`, `nguon` excel/app, cờ `thieu_minh_chung` sinh tự động; CHECK "Có hạn cụ thể ⇒ hạn hoặc lý do" trừ dòng Excel đã xong, "Hoàn thành ⇒ ngày hoàn thành" trừ Excel); `kl_lich_su` (trigger ghi từng cột đổi; `nguon` excel/app/dinh_chinh); `kl_chi_dao` (5 loại, bảng + RLS; hàm ở GĐ10); `kl_dinh_chinh` (đề nghị → duyệt bởi `quan_tri_kl` khác người đề nghị).
  - **`kl_trang_thai(nv, ngay)` là nguồn duy nhất của trạng thái** (kiểu `kl_trang_thai_kq`: `trang_thai`, `so_ngay_qua`, `ket_qua` đúng hạn/trễ/không đánh giá, `so_ngay_tre`, `do_tre_nhap_lieu`, `dang_dinh_chinh`, `nhom_dem` — ô Quá hạn không đếm dòng đang đính chính). Wrapper `kl_tinh_trang_thai(id, ngay)`; view `v_kl_dashboard` (security_invoker). "Hôm nay" = `kl_hom_nay()` theo giờ Việt Nam (Postgres trên Supabase chạy UTC).
  - RLS quyết định 7: `kl_pham_vi(chu_tri)` = `quan_tri_kl` ∨ chủ trì là mình ∨ Chánh VP ∨ PCVP có `phu_trach(phòng của chủ trì)` ∨ A2 cùng phòng. Thêm hội nghị/nhiệm vụ chỉ `quan_tri_kl`; chủ trì không có `quan_tri_kl` chỉ sửa tiến độ/hạn/lý do/ngày hoàn thành/minh chứng/văn bản/ghi chú (trigger); chỉ đạo chỉ A1/A2 trong phạm vi; lịch sử chỉ trigger ghi; Realtime bật cho `kl_nhiem_vu`, `kl_chi_dao`, `kl_dinh_chinh`.

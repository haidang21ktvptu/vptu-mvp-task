...
- **Xử lý:** thêm lĩnh vực bằng nút "Thêm lĩnh vực" (hộp lý do bắt buộc; hàm `admin_them_linh_vuc` ghi nhật ký — xem bảng "Nhật ký danh mục" ngay dưới). Nếu lĩnh vực mới phải thuộc người kiêm nhiệm: chủ dự án (`quan_tri_he_thong`) → Quản trị → dòng PCVP → **Kiêm nhiệm lĩnh vực** → chọn phòng, ngành, tích lĩnh vực mới, lý do, Lưu — có hiệu lực ngay, việc chuyển sang người kiêm nhiệm từ ngày chọn. Không vào được app: `supabase db query --linked --project-ref frwyxcmbonjaimziiuqr "select public.admin_phan_cong_phong('<username PCVP>', '<mã phòng>', true, 'Qua CLI: <lý do>', date '<YYYY-MM-DD hiệu lực>', '<mã ngành>', '<mã lĩnh vực>')"`. Nếu lĩnh vực mới đúng là của PCVP phụ trách phòng: không làm gì.
- **Khôi phục từ backup?** Không.

## 13. PR phát hành bị merge TRƯỚC khi gắn tag (sự cố #48, 2026-09-15)
- **Dấu hiệu:** PR `feature/phat-hanh-v2.x.y` đã vào `main` nhưng `git tag -l v2.x.y` trống, không có run `deploy-prod`, `phien-ban.json` bản live vẫn ghi phiên bản cũ, production chưa có migration mới (ví dụ #48: `be45313` đã merge, production vẫn `v2.2.0`, 0021 chưa áp).
- **Nguyên nhân:** quy trình mục 7 `docs/kien-truc.md` cần chủ dự án bấm merge **sau** khi tag; nút merge lại sẵn ngay khi CI xanh, mô tả PR không có chốt chặn nào nhắc "chưa tag" — sự cố #48 xảy ra đúng lúc chuyển từ bước "CI xanh, mời xác nhận backup + migration" sang bước tag. Không phải lỗi pipeline.
- **Xử lý (bài học rc1 — KHÔNG tag lên commit cũ):** commit đã lên `main` thì `deploy-staging` đã deploy sha đó, GitHub Pages không nhận lại → tag lên commit ấy sẽ "xanh" mà bản live không đổi (`kiem-tra` cũng chặn). Làm lại từ đầu với một commit mới: (1) từ `main` hiện tại tạo nhánh `feature/phat-hanh-v2.x.y-lan-2`; (2) một commit **chỉ tài liệu** (ghi sự cố vào file này, CHANGELOG, TRANG-THAI), không mã, không test; (3) mở PR, CI xanh; (4) backup tay, xác nhận migration trong phiên; (5) tag `v2.x.y` lên đầu nhánh mới (số tag giữ nguyên vì chưa từng đẩy); (6) duyệt deploy-prod → smoke → tag `production`; (7) **rồi mới** merge. Migration chưa áp thì không cần khôi phục gì.
- **Cách tránh (chốt chặn từ 2026-09-15):** (a) mô tả mọi PR phát hành mở đầu bằng dòng đậm **"KHÔNG BẤM MERGE cho tới khi tag `production` đã gắn — xem Actions run của tag `v*`"** và checklist ba ô: backup tay ✓ · tag `v*` đã đẩy ✓ · tag `production` đã gắn ✓; (b) Claude chỉ báo "mời merge" cho PR phát hành **sau khi** tag `production` xuất hiện (`git ls-remote --tags origin production`), khác với PR thường (báo khi CI xanh); (c) khi cần chủ dự án xác nhận backup/migration, nói rõ "chưa merge" ngay trong cùng câu.
- **Khôi phục từ backup?** Không — chưa có gì thay đổi trên production.

# TRẠNG THÁI DỰ ÁN — bản bàn giao (cập nhật: 2026-09-16, `v2.3.0` đã lên production; **định hướng lại theo phụ lục Công văn 1400** — bộ tài liệu v3 đã lập, **15 câu hỏi nghiệp vụ đã chốt 16/9**, sẵn sàng mở GĐ14 / migration 0022)

Đọc trước: `CLAUDE.md` (quy tắc), **`docs/MUC-TIEU-1400.md` (thước đo mọi giai đoạn)**, `docs/SPEC.md` (v3, nghiệp vụ), `docs/RA-SOAT-HIEN-TRANG.md` (giữ/sửa/hợp nhất/bỏ), `docs/CAU-HOI-NGHIEP-VU.md` (15 câu chờ chủ dự án), `docs/LO-TRINH-V3.md` (GĐ14–19, migration từ 0022), `docs/DESIGN.md` (giao diện), `docs/kien-truc.md` (pipeline, secret, quay lui), `docs/PROMPTS.md` (prompt theo giai đoạn). Tra cứu: `docs/SPEC-v2-luu.md` (mã yêu cầu cũ), `docs/thiet-ke-theo-doi-kl-btvtu.md` (thiết kế GĐ8–10, đóng băng). Lịch sử: `CHANGELOG.md` mục 7–20.
...
- Backend chỉ Supabase: Auth (email quy ước `<username>@vptu.caobang.local`), Postgres + RLS + hàm `security definer` (`assign_task`, `approve_task`, `submit_evidence`, `warn_task`, `mark_*_read`), Realtime `postgres_changes`. Schema = `supabase/migrations/0001–0021` (staging và production đều ở 0021 = `v2.3.0`, phát hành 2026-09-15; dữ liệu KL trên production: 185 nhiệm vụ, 51 dòng đã có `linh_vuc_ma`), không sửa tay Dashboard. Migration kế tiếp đánh số từ **0022** (`LO-TRINH-V3.md`).
...
- Pipeline (rút gọn PR #50, 16/9): `ci.yml` (PR: gitleaks, phân loại PR, migration+lint+RLS local, build+lint+300 dòng, e2e trên staging — 8 lượt đăng nhập/lần; **PR chỉ đổi `docs/**`/`*.md` bỏ qua hai job chậm bằng job rỗng cùng tên**, nhánh `*phat-hanh*` luôn chạy đủ; Chromium cache) → `deploy-staging.yml` (push main: db push staging → RLS token thật ‖ build 2 bản → Pages) → `deploy-prod.yml` (tag `v*` trên đầu nhánh phát hành: `kiem-tra` → duyệt → backup gpg → db push production → build → Pages → smoke `smoke_test` → gắn tag `production`). GitHub Pages chỉ nhận mỗi commit một lần (`pages_build_version` = sha OIDC) — vì thế tag phải gắn trước khi merge.
...
## 6. Việc còn lại
0. **Định hướng lại theo 1400 — bước kế tiếp**: (a) ~~chốt câu hỏi~~ **đã chốt 16/9** (`CAU-HOI-NGHIEP-VU.md`, SPEC/LO-TRINH đã sửa theo); (b) sau khi #51 merge: mở **GĐ14 PR 14A** (migration `0022_thuc_the_thong_nhat_cot.sql`, additive: `dm_don_vi` 13 + 5 phòng, `dm_san_pham`, `dm_cap`, cột Owner/Product/cấp/ngày nhận/`nhiem_vu_cha`/`theo_1400`, 3 khoá `kl_cau_hinh`) theo `LO-TRINH-V3.md`; (c) không có gói Pro: cảnh báo bằng workflow cron (GĐ16), tệp minh chứng chờ kinh phí (mục "Chờ điều kiện" của lộ trình). Điều kiện "chạy song song 2 kỳ + ≥ 9 chuyên viên" của GĐ10 thay bằng CH-15 (một kỳ đối chiếu số tổng sau hợp nhất) — **đã đồng ý**.
...
4. **GĐ14–17 mới** (`LO-TRINH-V3.md`): 14 thực thể thống nhất (0022–0023, `v3.0.0`); 15 minh chứng có cấu trúc + đóng nhiệm vụ (0024, không tệp); 16 cảnh báo tự động 3 cấp bằng workflow cron GitHub (0025); 17 dashboard ngoại lệ 4 trường + vai trò `A0` Thường trực (0026–0027). Ước 18–21 ngày làm việc.
5. (gộp vào 3–4)

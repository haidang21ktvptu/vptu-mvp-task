# TRẠNG THÁI DỰ ÁN — bản bàn giao (cập nhật: 2026-09-15, GĐ8 xong; GĐ9 PR 9A phân công lĩnh vực đang mở)
...
| 9 | Phân công theo lĩnh vực + dashboard KL (Phần 4 GĐ9, Phần 5.4) | **Đang làm**: **PR 9A** #38 (0018–0019, màn hình Quản trị kiêm nhiệm lĩnh vực + danh mục, `rls-11`) mở 2026-09-15 — 93/93 RLS local, 17/17 e2e local. Còn: 9B script ánh xạ lĩnh vực, 9C/9D dashboard |
...
## 6. Việc còn lại
1. **GĐ9 — PR 9A mở (2026-09-15, #38)** — phân công PCVP theo lĩnh vực (`0018`, `0019`, màn hình Quản trị: "Kiêm nhiệm lĩnh vực", "Danh mục lĩnh vực" cho `quan_tri_kl`; `rls-11` 8 case; thiết kế Phần 5.4 + quyết định 7 + `xu-ly-su-co.md` #12). Trên staging `rls-11` và e2e quản trị tự bỏ qua tới khi merge. **Sau merge:** deploy-staging áp 0018–0019 → chạy lại `tests/rls` trên staging (kỳ vọng 93/93) → phát hành **`v2.2.0`** (tag trước merge trên nhánh phát hành, xác nhận trong phiên; 5 dòng phân công PCVP ↔ phòng đang có trên production giữ nguyên = "cả phòng") → chủ dự án nhập kiêm nhiệm thật nếu cần. **PR 9B**: `scripts/anh-xa-linh-vuc.mjs` — đọc 82 giá trị `linh_vuc_chi_tiet` hiện có, đề xuất ánh xạ về `dm_linh_vuc`, xuất bảng để người quản trị sheet duyệt (file ngoài repo), rồi `--ghi` áp lên production; không ánh xạ được → NULL. Rồi dashboard đọc và chạy song song (thiết kế Phần 3, 4): PR 9C màn hình A3/A2 (danh sách theo chủ trì, cập nhật nhanh, bộ lọc hội nghị/ngành/trạng thái; mọi số đọc từ `v_kl_dashboard`; ngăn lỗi trên form theo Phần 6.8), PR 9D dashboard A1 (bố cục mục 3.2, truy vết Phần 6.3, chưa có nút chỉ đạo) + xuất HTML hai bản từ snapshot `kl_bao_cao` (bảng mới, migration 0020). Form nhập nhiệm vụ mới: chọn ngành → lĩnh vực khoá theo ngành, bắt buộc. Dữ liệu thật đã có trên production; local/staging dùng bộ vàng ẩn danh (cùng bộ số). Chạy song song 2 kỳ báo cáo với Google Sheet; trùng 2 kỳ + ≥ 9 chuyên viên tự cập nhật mới sang GĐ10.
2. **GĐ10** — vòng chỉ đạo và tắt Excel: PR 10A 4 hành động chỉ đạo (mục 3.3), ô "Chỉ đạo chưa phản hồi", thông báo qua `direct_messages`; PR 10B chốt bắt buộc (minh chứng khi Hoàn thành, "Có hạn cụ thể ⇒ hạn" chuyển sang chặn), Google Sheet chuyển chỉ đọc; bổ sung 6 tình huống vào `docs/xu-ly-su-co.md`. Xong khi một kỳ báo cáo hoàn toàn từ app và một chỉ đạo thật đi hết vòng.
3. **GĐ11** — xuất PDF hai bản trực tiếp từ app (có mã báo cáo), bảng chéo ngành × cơ quan trình, tuổi quá hạn theo bậc; sau 2 tháng vận hành rà lại ngưỡng 7 ngày, xem xét gộp ngành 1 và 12.
4. ~~Đổi ba mặc định `current_date` trong 0013 sang `kl_hom_nay()`~~ — **đã làm trong 0018 (PR 9A)**: `phu_trach(p_ngay)`, `admin_phan_cong_phong(p_tu_ngay)`, cột `phu_trach_phong.tu_ngay`; `rls-11` kiểm mặc định = `kl_hom_nay()`.
...
```
Phiên kế tiếp (sau merge PR 9A #38): kiểm deploy-staging áp 0018–0019 → `cd tests/rls && npm test` trên staging (kỳ vọng 93/93, `rls-11` chạy thật) → phát hành `v2.2.0` (mục 6.1, nhánh phát hành nhỏ, tag trước merge, xác nhận trong phiên) → PR 9B `scripts/anh-xa-linh-vuc.mjs` → PR 9C màn hình A3/A2 (đọc Phần 3.4, 6.3, 6.8, DESIGN.md; dữ liệu local: `supabase db reset` rồi `node scripts/nhap-kl-btvtu.mjs --file tests/rls/du-lieu-vang/kl-btvtu.json --local --ghi`). Plan mode, chờ duyệt.

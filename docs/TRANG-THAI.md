# TRẠNG THÁI DỰ ÁN — bản bàn giao (cập nhật: 2026-09-15, GĐ9 PR 9A merged #38, PR 9B đang mở)
...
| 9 | Phân công theo lĩnh vực (Phần 4 GĐ9, Phần 5.4) | PR 9A #38 merged (staging 0019, `tests/rls` 93/93). **PR 9B đang mở** (0020: đính chính `linh_vuc_ma`, `dm_lich_su`, hàm danh mục có nhật ký; `scripts/anh-xa-linh-vuc.mjs`; 101/101 local). Còn: merge 9B → phát hành `v2.2.0` → duyệt CSV → `--ghi --production` = đóng GĐ9 |
...
## 6. Việc còn lại
1. **GĐ9 — phân công theo lĩnh vực.** PR 9A #38 merged 15/9; deploy-staging áp 0018–0019, `tests/rls` staging 93/93. **PR 9B đang mở** (`0020_linh_vuc_dinh_chinh_nhat_ky.sql`; `scripts/anh-xa-linh-vuc.mjs`; màn hình danh mục qua hàm có lý do + bảng nhật ký; `kl-anh-xa-linh-vuc` + `rls-11` viết lại; 101/101 local — staging sẽ 99 + 2 skip). **Đóng GĐ9 sau merge 9B, theo thứ tự:** (a) chạy `tests/rls` trên staging (kỳ vọng 99 pass + 2 skip; `dm_lich_su`, hàm danh mục chạy thật); (b) phát hành **`v2.2.0`** = 0018–0020 (nhánh phát hành nhỏ, tag trước merge, xác nhận trong phiên, backup trước); (c) người quản trị sheet duyệt `vptu-backup/nguon-kl-btvtu/anh-xa-linh-vuc.csv` (đã sinh 15/9 bằng dry-run chỉ đọc: 84 cặp, 9 đề xuất, 14 gợi ý) — sau khi production có 0018, chạy lại dry-run để CSV dùng danh mục thật; (d) `node scripts/anh-xa-linh-vuc.mjs --project-ref frwyxcmbonjaimziiuqr --production --ghi --file "<csv đã duyệt>"` — bước riêng ngoài pipeline, backup-db.sh trước, xác nhận trong phiên, in báo cáo trước/sau; (e) chép biên bản (không họ tên) từ `scripts/out/` vào `docs/bien-ban-nhap-kl-btvtu.md` qua PR; (f) chủ dự án nhập kiêm nhiệm thật (nếu có) trên màn hình Quản trị. Xong GĐ9.
...
```
Phiên kế tiếp (sau khi chủ dự án merge PR 9B): làm theo mục 6.1 (a)→(f) — test staging → phát hành `v2.2.0` (xác nhận trong phiên) → duyệt CSV → `--ghi --production` (xác nhận trong phiên, backup trước) → biên bản → đóng GĐ9; rồi GĐ10 PR 10A (đọc Phần 3.4, 6.3, 6.8, DESIGN.md; dữ liệu local: `supabase db reset` rồi `node scripts/nhap-kl-btvtu.mjs --file tests/rls/du-lieu-vang/kl-btvtu.json --local --ghi`). Plan mode, chờ duyệt.

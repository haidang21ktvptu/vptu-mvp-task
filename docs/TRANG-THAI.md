# TRẠNG THÁI DỰ ÁN — bản bàn giao (cập nhật: 2026-09-15, GĐ8 xong — v2.1.0 trên production)
...
- Backend chỉ Supabase: Auth (email quy ước `<username>@vptu.caobang.local`), Postgres + RLS + hàm `security definer` (`assign_task`, `approve_task`, `submit_evidence`, `warn_task`, `mark_*_read`), Realtime `postgres_changes`. Schema = `supabase/migrations/0001–0017` (production và staging đều ở 0017), không sửa tay Dashboard.
...
## 6. Việc còn lại
1. **GĐ9 — việc kế tiếp** — dashboard đọc và chạy song song (thiết kế Phần 3, 4): PR 9A màn hình A3/A2 (danh sách theo chủ trì, cập nhật nhanh, bộ lọc hội nghị/ngành/trạng thái; mọi số đọc từ `v_kl_dashboard`; ngăn lỗi trên form theo Phần 6.8), PR 9B dashboard A1 (bố cục mục 3.2, truy vết Phần 6.3, chưa có nút chỉ đạo) + xuất HTML hai bản từ snapshot `kl_bao_cao` (bảng mới, migration 0018). Dữ liệu thật đã có trên production; local/staging dùng bộ vàng ẩn danh (cùng bộ số). Chạy song song 2 kỳ báo cáo với Google Sheet; trùng 2 kỳ + ≥ 9 chuyên viên tự cập nhật mới sang GĐ10.
...
```
Phiên kế tiếp: GĐ9 PR 9A — đọc CLAUDE.md, TRANG-THAI, `docs/thiet-ke-theo-doi-kl-btvtu.md` Phần 3.4, 6.3, 6.8 và DESIGN.md; dữ liệu local: `supabase db reset` rồi `node scripts/nhap-kl-btvtu.mjs --file tests/rls/du-lieu-vang/kl-btvtu.json --local --ghi`. Plan mode, chờ duyệt.

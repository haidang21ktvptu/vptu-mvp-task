# TRẠNG THÁI DỰ ÁN — bản bàn giao (cập nhật: 2026-09-15, v2.2.0 trên production; GĐ9 còn bước ánh xạ lĩnh vực)
...
| 9 | Phân công theo lĩnh vực (Phần 4 GĐ9, Phần 5.4) | PR 9A #38, 9B #39 merged; **`v2.2.0` (0018–0020) trên production 15/9** (#40). Còn: người quản trị sheet duyệt file Excel ánh xạ lĩnh vực → `anh-xa-linh-vuc.mjs --ghi --production` (bước riêng, backup + xác nhận trong phiên) → biên bản = đóng GĐ9 |
...
## 6. Việc còn lại
1. **GĐ9 — phân công theo lĩnh vực.** PR 9A #38 merged 15/9; deploy-staging áp 0018–0019, `tests/rls` staging 93/93. **PR 9B #39 merged 15/9**; deploy-staging áp 0020, `tests/rls` staging **99/99** (a ✓). **(b) ✓ phát hành `v2.2.0`** = 0018–0020 (PR #40, tag trước merge, backup tay `prod-20260915-0823-tay`; bản live ghi `v2.2.0`, production ở 0020, 5 phân công vẫn "cả phòng"). Còn lại theo thứ tự: (c) người quản trị sheet duyệt trên **Excel**: dry-run chỉ đọc sau `v2.2.0` (danh mục thật) đã sinh `vptu-backup/nguon-kl-btvtu/anh-xa-linh-vuc.2026-09-15.csv` — 84 cặp, 9 đề xuất, 14 gợi ý, 61 trống (trùng bản trước phát hành); người duyệt điền cột E "linh_vuc_chot" trên file .xlsx (sheet "Đối chiếu lĩnh vực", tiêu đề dòng 5 — script đọc được .xlsx từ PR bổ sung 9B, xuất sẵn bằng `--out "<file>.xlsx"` nếu cần); (d) `node scripts/anh-xa-linh-vuc.mjs --project-ref frwyxcmbonjaimziiuqr --production --ghi --file "<xlsx hoặc csv đã duyệt>"` — bước riêng ngoài pipeline, backup-db.sh trước, xác nhận trong phiên, in báo cáo trước/sau; (e) chép biên bản (không họ tên) từ `scripts/out/` vào `docs/bien-ban-nhap-kl-btvtu.md` qua PR; (f) chủ dự án nhập kiêm nhiệm thật (nếu có) trên màn hình Quản trị. Xong GĐ9.
...
## 7. Lệnh để tiếp tục
```
cd tests/e2e && npm test      # 17 test e2e trên staging (8 lượt đăng nhập); cd tests/rls && npm test (101 test local / 99 + 2 skip staging, 7 lượt); RLS_LOCAL=1 / E2E_LOCAL=1 để chạy trên Supabase local; npm run lint ở gốc
```
...
```
Phiên kế tiếp: khi có file .xlsx đã duyệt → mục 6.1 (d) `node scripts/anh-xa-linh-vuc.mjs --project-ref frwyxcmbonjaimziiuqr --production --ghi --file "<xlsx>"` (backup-db.sh trước, xác nhận trong phiên, đọc kỹ báo cáo "trước khi ghi") → (e) biên bản vào `docs/bien-ban-nhap-kl-btvtu.md` qua PR → đóng GĐ9; rồi GĐ10 PR 10A (đọc Phần 3.4, 6.3, 6.8, DESIGN.md; dữ liệu local: `supabase db reset` rồi `node scripts/nhap-kl-btvtu.mjs --file tests/rls/du-lieu-vang/kl-btvtu.json --local --ghi`). Plan mode, chờ duyệt.

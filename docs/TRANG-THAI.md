# TRẠNG THÁI DỰ ÁN — bản bàn giao (cập nhật: 2026-09-15, GĐ9 PR 9A/9B merged; phát hành v2.2.0 đang thực hiện)
...
## 6. Việc còn lại
1. **GĐ9 — phân công theo lĩnh vực.** PR 9A #38 merged 15/9; deploy-staging áp 0018–0019, `tests/rls` staging 93/93. **PR 9B #39 merged 15/9**; deploy-staging áp 0020, `tests/rls` staging **99/99** (a ✓). **(b) đang làm: phát hành `v2.2.0`** = 0018–0020 từ nhánh `feature/phat-hanh-v2.2.0` (chủ dự án xác nhận trong phiên 15/9, backup tay `prod-20260915-0823-tay` xong trước khi tag). Còn lại theo thứ tự: (c) người quản trị sheet duyệt `vptu-backup/nguon-kl-btvtu/anh-xa-linh-vuc.csv` (đã sinh 15/9 bằng dry-run chỉ đọc: 84 cặp, 9 đề xuất, 14 gợi ý) — sau khi production có 0018, chạy lại dry-run để CSV dùng danh mục thật; (d) `node scripts/anh-xa-linh-vuc.mjs --project-ref frwyxcmbonjaimziiuqr --production --ghi --file "<csv đã duyệt>"` — bước riêng ngoài pipeline, backup-db.sh trước, xác nhận trong phiên, in báo cáo trước/sau; (e) chép biên bản (không họ tên) từ `scripts/out/` vào `docs/bien-ban-nhap-kl-btvtu.md` qua PR; (f) chủ dự án nhập kiêm nhiệm thật (nếu có) trên màn hình Quản trị. Xong GĐ9.

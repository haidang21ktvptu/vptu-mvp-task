# TRẠNG THÁI DỰ ÁN — bản bàn giao (cập nhật: 2026-09-15, GĐ8 PR 8B mở)
...
| 8 | Theo dõi KL BTVTU — mô hình dữ liệu và nhập liệu (`docs/thiet-ke-theo-doi-kl-btvtu.md` Phần 4) | **Đang làm**: PR 8A-1 merged #33, PR 8A-2 merged #34; **PR 8B** (0017 + `scripts/nhap-kl-btvtu.mjs` + bộ vàng ẩn danh, `kl-moc` chạy thật trên local/CI) mở 2026-09-15. Còn: nạp bộ vàng vào staging, phát hành `v2.1.0` (schema), nhập dữ liệu thật (bước riêng) |
...
- Backend chỉ Supabase: Auth (email quy ước `<username>@vptu.caobang.local`), Postgres + RLS + hàm `security definer` (`assign_task`, `approve_task`, `submit_evidence`, `warn_task`, `mark_*_read`), Realtime `postgres_changes`. Schema = `supabase/migrations/0001–0017` (0017 chờ merge PR 8B; production mới tới 0012), không sửa tay Dashboard.
...
## 6. Việc còn lại
1. **GĐ8 — PR 8B mở (2026-09-15)**: `0017` (tên loại hạn nguyên văn), `scripts/nhap-kl-btvtu.mjs` + `scripts/kl/` (dry-run mặc định, `--ghi`, `--production`, `--xoa-cu` chỉ local/staging, `--anh-xa-nguoi-sua`), `scripts/an-danh-kl-btvtu.mjs` → `tests/rls/du-lieu-vang/kl-btvtu.json`; CI job local nạp bộ vàng nên `kl-moc-2026-09-14` chạy thật (146/16/8/6/6/3/0 = 185, 85/85 local). File gốc đã sửa dòng hạn-trước-ngày-ban-hành; dry-run file thật trên local = 0 vi phạm dữ liệu. **Sau khi merge 8B**: nạp bộ vàng vào staging `node scripts/nhap-kl-btvtu.mjs --file tests/rls/du-lieu-vang/kl-btvtu.json --project-ref vojmrjezspdftovzinek --ghi` (deploy-staging đã áp 0017) → `cd tests/rls && npm test` trên staging (`kl-moc` không còn skip).
2. **Production — HAI BƯỚC TÁCH BIỆT, không gộp:**
   - **(a) Phát hành `v2.1.0` = schema 0013–0017** (0013–0016 đã ở `main` từ #33/#34 nên lần phát hành kế tiếp áp chúng dù có 8B hay không): tag trên đầu nhánh phát hành đã CI xanh, TRƯỚC khi merge → duyệt environment → backup + `db push` → smoke → merge. Sau bước này production có schema KL nhưng **chưa có dữ liệu KL**. Rồi chủ dự án đăng nhập `buibahaidang` → Quản trị hệ thống → nhập phân công PCVP ↔ phòng (`phu_trach_phong` production đang rỗng) và cấp `quan_tri_kl` cho 2 người.
   - **(b) Nhập dữ liệu thật** — bước riêng, **không nằm trong pipeline phát hành**, chạy tay từ máy chủ dự án, cần xác nhận trong phiên (rule 12): `node scripts/nhap-kl-btvtu.mjs --file "<xlsx ngoài repo>" --project-ref frwyxcmbonjaimziiuqr --production --anh-xa-nguoi-sua "<json ngoài repo>"` (dry-run) → 0 vi phạm, xác nhận bảng ánh xạ 8 tên + VPTU → `bash scripts/backup-db.sh --project-ref frwyxcmbonjaimziiuqr --thu-muc "/d/TU 2026/Project/vptu-backup"` → thêm `--ghi` → kiểm `v_kl_dashboard` (146/16/8/6/6/3/0 tại 14/9) → chép biên bản từ `scripts/out/` vào `docs/bien-ban-nhap-kl-btvtu.md` (không họ tên) qua PR. Xong GĐ8.
...
6. **Đổi ba mặc định `current_date` trong 0013** (`phu_trach(p_ngay)`, cột `phu_trach_phong.tu_ngay`, `admin_phan_cong_phong(p_tu_ngay)`) sang `kl_hom_nay()` — gộp vào một PR sau, **không gấp** (chỉ lệch khi gọi CLI trong khung 0h–7h VN; màn hình Quản trị luôn gửi ngày người nhập chọn, `kl_pham_vi` đã truyền `kl_hom_nay()` tường minh; quyết định 15/9).
...
```
Phiên kế tiếp (sau merge PR 8B): nạp bộ vàng vào staging + chạy tests/rls trên staging (mục 6.1); rồi phát hành `v2.1.0` (mục 6.2a) trên một nhánh phát hành nhỏ; nhập dữ liệu thật (mục 6.2b) chỉ khi chủ dự án xác nhận trong phiên, có backup trước.

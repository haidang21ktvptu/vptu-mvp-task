# TRẠNG THÁI DỰ ÁN — bản bàn giao (cập nhật: 2026-09-16, `v2.3.0` đã lên production; **định hướng lại theo phụ lục Công văn 1400** — bộ tài liệu v3 đã lập, chờ chủ dự án chốt câu hỏi nghiệp vụ trước migration 0022)
...
## 6. Việc còn lại
0. **Định hướng lại theo 1400 — bước kế tiếp**: (a) chủ dự án điền ô "Quyết định" trong `docs/CAU-HOI-NGHIEP-VU.md` (tối thiểu CH-1, 2, 5, 6, 8, 9, 14 — điều kiện T1 của lộ trình) qua một PR docs-only; (b) sửa `SPEC.md` v3 tại các chỗ đánh dấu `[CH-n]` quyết khác đề xuất; (c) mở GĐ14 PR 14A (migration `0022_thuc_the_thong_nhat_cot.sql`, additive) theo `LO-TRINH-V3.md`; (d) việc ngoài code: lên gói Pro để có `pg_cron` + Storage (GĐ15–16). Điều kiện "chạy song song 2 kỳ + ≥ 9 chuyên viên" của GĐ10 thay bằng CH-15 (một kỳ đối chiếu số tổng sau hợp nhất) — chờ chủ dự án xác nhận.
...
4. **GĐ14–17 mới** (`LO-TRINH-V3.md`): 14 thực thể thống nhất (0022–0023, `v3.0.0`); 15 minh chứng tệp + đóng nhiệm vụ (0024–0025, cần Storage); 16 cảnh báo tự động 3 cấp (0026, cần `pg_cron` gói Pro); 17 dashboard ngoại lệ 4 trường + cấp Thường trực (0027, +0028 nếu có vai trò A0). Ước 19–22 ngày làm việc.
5. (gộp vào 3–4)

...
**PR 9B — ánh xạ lĩnh vực cho dữ liệu cũ, đính chính `linh_vuc_ma`, danh mục có nhật ký (làm 15/9/2026, migration 0020)**: (1) `scripts/anh-xa-linh-vuc.mjs` — dry-run **chỉ đọc** 185 dòng production, gom ~84 cặp (ngành, `linh_vuc_chi_tiet`), **đề xuất chỉ khi bằng nhau sau chuẩn hoá** (bỏ dấu, hoa/thường, khoảng trắng) với lĩnh vực **thuộc đúng ngành của dòng**; giá trị chỉ *chứa* tên lĩnh vực không được đề xuất mà ghi "gợi ý" ở cột ghi chú; xuất `vptu-backup/nguon-kl-btvtu/anh-xa-linh-vuc.csv` (ngoài repo; cột `gia_tri_goc; nganh; so_dong; linh_vuc_de_xuat; linh_vuc_chot; ghi_chu`, UTF-8 BOM, phân cách `;`) cho người quản trị sheet điền `linh_vuc_chot` (tên hoặc mã) — người duyệt dùng Excel theo vùng dấu phẩy nên file duyệt thực tế là **.xlsx** (sheet "Đối chiếu lĩnh vực", tiêu đề dòng 5, cột E = chốt, cột B tên ngành hiển thị → mã; `--out *.xlsx` xuất cùng bố cục); `--ghi --file` nhận .csv hoặc .xlsx, chỉ điền dòng có chốt, dòng trống giữ NULL, chốt sai ngành → dừng không ghi; ghi bằng một khối SQL tắt tạm trigger `b_kl_nhiem_vu_truoc_ghi` để **không đổi `cap_nhat_luc`** của dòng (giữ đúng chỉ số "không cập nhật 30 ngày"), `kl_lich_su` vẫn ghi từng dòng với `nguoi_sua_ghi_chu` của script; production cần `--production`, backup và xác nhận trong phiên; đích chưa có 0018 vẫn dry-run được bằng danh mục đọc từ local. (2) `kl_dinh_chinh.cot` nhận `linh_vuc_ma` (FK ghép chặn sai ngành ngay lúc duyệt). (3) Thêm/sửa `dm_linh_vuc` **chỉ qua** `admin_them_linh_vuc` / `admin_sua_linh_vuc` (lý do bắt buộc, ghi `dm_lich_su` cùng transaction, mã sinh phía server bằng `unaccent`; thu quyền ghi thẳng bảng của 0018); màn hình Quản trị có hộp lý do và bảng "Nhật ký danh mục". Test: `tests/rls/kl-anh-xa-linh-vuc` (đề xuất không vượt ngành, CSV, chạy thật trên local: `--ghi` không đụng dòng trống, không đổi `cap_nhat_luc`), `rls-11` phần danh mục viết lại.
...
- **Danh mục `dm_linh_vuc`** (`ma`, `nganh_ma` → `dm_nganh`, `ten`, `thu_tu`): mỗi lĩnh vực thuộc đúng một ngành. Danh mục ban đầu tách từ tên 12 ngành theo dấu " - " (32 lĩnh vực; ví dụ ngành 8 → Kinh tế tổng hợp / Tài chính / Đầu tư / Ngân sách); migration tự kiểm ghép lại phải ra đúng nguyên văn tên ngành. Người có `quan_tri_kl` thêm lĩnh vực và sửa tên/thứ tự trên màn hình Quản trị (phần "Danh mục lĩnh vực", chỉ hiện với cờ này) — **chỉ qua hàm `admin_them_linh_vuc` / `admin_sua_linh_vuc` (0020) với lý do bắt buộc, ghi nhật ký `dm_lich_su` cùng transaction** (PR 9A tạm cho ghi thẳng bảng, PR 9B thu lại theo nguyên tắc "mọi thay đổi quản trị đều có vết"); mã do server sinh (`LV<số ngành>_<tên bỏ dấu>`); không xoá, không chuyển ngành vì đã có việc và phân công tham chiếu. Dữ liệu cũ (82 giá trị `linh_vuc_chi_tiet` trên production) ánh xạ bằng `scripts/anh-xa-linh-vuc.mjs` có người duyệt (Phần 4 PR 9B).
...
- Người có `quan_tri_kl` (không phải người đề nghị) duyệt hoặc bác bỏ, có ghi lý do. Duyệt → hệ thống áp giá trị mới, ghi `kl_lich_su` với loại "đính chính đã duyệt" trỏ về đề nghị. Cột được đính chính (`kl_dinh_chinh.cot`): hạn xử lý, loại thời hạn, tiến độ, ngày hoàn thành, minh chứng, ngành, cơ quan trình, lý do chưa có hạn và **lĩnh vực** (`linh_vuc_ma`, từ 0020 — FK ghép chặn lĩnh vực sai ngành ngay lúc duyệt, đề nghị giữ trạng thái chờ để sửa lại).

...
export function cellHtml(pcvp, phong) {
  ...
  const nut = `<button type="button" class="btn btn-nho ${on ? 'btn-cham' : 'btn-phu'}" role="switch" aria-checked="${on}"
  ...
  const chips = dongKiemNhiem(pcvp.id, phong).map((r) => `<span class="inline-flex items-center gap-1">
      <span class="muc muc-vang" title="Kiêm nhiệm từ ${escapeHtml(r.tu_ngay)}">Kiêm nhiệm: ${escapeHtml(nhanNganhLinhVuc(r.nganh_ma, r.linh_vuc_ma))}</span>
      <button type="button" class="btn btn-nho btn-phu" data-action="ketThucKiemNhiem" data-username="${escapeHtml(pcvp.username)}"

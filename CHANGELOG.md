...
- **PR 10B (2026-09-15)** — màn hình **"Kết luận BTVTU"** dùng chung mọi vai trò (`frontend/src/views/shared/kl/`, mục thanh bên mới, section `viewKl`): một lần đọc `v_kl_dashboard` (RLS `kl_pham_vi` 0019 lọc phạm vi — frontend không lọc theo vai/phòng) → thanh 9 ô số theo `nhom_dem` là nút lọc (tổng các ô = số dòng, ô "Đang đính chính" chỉ hiện khi > 0), bộ lọc hội nghị/ngành/lĩnh vực (có "Chưa phân loại" = lĩnh vực NULL, "Chưa có ngành") + tìm mã/nội dung, bảng xếp theo 3.4 (chỉ đạo chờ → quá hạn nhiều ngày → sắp hạn → cần điền hạn → đang làm → chờ điều kiện → thường xuyên → hoàn thành), dấu "Số liệu tính đến hh:mm:ss", chip bộ lọc do dashboard truyền sang (`setKlLoc`, dùng ở 10C). **Cập nhật nhanh** (nút chỉ hiện cho chủ trì/`quan_tri_kl`; quyền thật ở policy 0016 + guard 0015): đúng 7 cột được phép, hạn Ký ban hành khoá, ô "chưa xác định được hạn" + lý do, Hoàn thành bắt buộc ngày + minh chứng (kiểm ở form bằng tiếng Việt, DB là chốt), "còn/quá N ngày" tức thì, dán minh chứng có "d/m/yyyy" → gợi ý ngày hoàn thành (6.8), ngày tối đa = `kl_hom_nay()` từ DB. **Ngăn chi tiết truy vết** (6.3): mỗi trường then chốt = giá trị · nhập bởi ai · lúc nào · nguồn (`kl_lich_su`), nguồn dòng (Excel: "không có ngày hoàn thành gốc"), đính chính chờ, chỉ đạo chờ, lịch sử đầy đủ. Không có nút chỉ đạo/đính chính (GĐ11).
- Module thuần `frontend/src/lib/kl/` (`tong-hop.js` đếm/nhóm/lọc/sắp xếp/chất lượng dữ liệu — dùng chung cho 10C; `nhan.js`; `ngay.js` — "hôm nay" theo Asia/Ho_Chi_Minh; `du-lieu.js`); unit test `frontend/tests/tong-hop.test.mjs` (11 case: bộ số 146/16/8/6/6/3/0 = 185, bất biến tổng nhóm/lĩnh vực, "Chưa phân loại" 134 (đo production 15/9), sắp xếp, múi giờ 17h–23h UTC) chạy `npm test` trong `frontend/` và ở job "Build frontend" của CI. Test RLS `kl-pham-vi-tong-hop` (7 vai: tập dòng = kỳ vọng tính từ `accounts` + `phu_trach_phong`, tổng nhóm = tổng dòng, lĩnh vực NULL không mất; chạy khi kiêm nhiệm rỗng và có kiêm nhiệm). e2e kịch bản 9 `kl-chuyen-vien.spec.js` 4 case × 2 kích thước (0 lượt đăng nhập thêm; gồm việc "Cần điền hạn" hoàn thành không cần hạn). Đã kiểm tra tay trên local với bộ vàng (cv1: 52 dòng, cập nhật Hoàn thành, 360px). Ảnh (dữ liệu giả): `docs/anh-man-hinh/gd10/`.
- **PR 10C (2026-09-15)** — dashboard **"Tổng quan KL BTVTU"** của A1 (`frontend/src/views/a1/kl-dashboard/`, mục thanh bên A1, section `viewKlDashboard`; thiết kế 3.2 đã chốt: không "so với kỳ trước", ô chỉ đạo → GĐ11, hàng 4 = ngành → lĩnh vực). Cùng nguồn đọc với 10B (`v_kl_dashboard`, RLS lọc: CVP tất cả, PCVP theo phụ trách/kiêm nhiệm), tổng hợp bằng `lib/kl/tong-hop.js`. Hàng 1 "cần can thiệp" chỉ hiện ô > 0 (quá hạn, đang đính chính, sắp đến hạn N ngày từ `kl_cau_hinh`, cần điền hạn + tuổi lớn nhất, chờ điều kiện); hàng 2 tổng / hoàn thành (tỷ lệ, n chưa minh chứng) / đang thực hiện / thường xuyên; hàng 3 thanh xếp chồng theo chủ trì (chỉ việc đang mở) và tỷ lệ hoàn thành 8 hội nghị gần nhất (HTML/CSS, không thư viện); hàng 4 bảng ngành → lĩnh vực có "Chưa phân loại" và "Chưa có ngành"; cuối trang chất lượng số liệu (6.7: chưa minh chứng, không ngày gốc, không cập nhật > N ngày, đính chính chờ, độ trễ nhập liệu TB/max). **Mọi con số/đoạn/ô là nút** mở danh sách 10B với bộ lọc tương ứng (`openKl(loc)`), có nút "← Về tổng quan"; thêm bộ lọc `nhomTrong`, `dangDinhChinh`. Lưu ý kỹ thuật: tên lớp CSS trong `@layer components` phải xuất hiện nguyên văn trong mã nguồn (Tailwind cắt lớp không thấy) — `LOP_DOAN`.
- Test: e2e kịch bản 10 `kl-dashboard.spec.js` 5 case × 2 kích thước (tổng các ô = ô Tổng; ô Tổng/Quá hạn/"Chưa phân loại"/đoạn chủ trì → đúng số dòng, đúng nhóm, đúng chip; 0 lượt đăng nhập thêm), local 8/8; unit test 10B đã phủ tổng hợp. Kiểm tra tay local với bộ vàng (`demo_cvp`: 8/3/6 · 185/146/6/16 = 185). Ảnh: `docs/anh-man-hinh/gd10/kl-dashboard-*.png`.
- **PR 10D (2026-09-15)** — **thời gian thực** cho hai màn hình KL (`frontend/src/features/kl-realtime.js`, thiết kế 3.5, quyết định 8): kênh Supabase Realtime `kl_feed` nghe `postgres_changes` trên `kl_nhiem_vu` / `kl_chi_dao` / `kl_dinh_chinh` (publication 0016; RLS lọc sự kiện theo người nghe); sự kiện chỉ là tín hiệu — gộp 500 ms rồi **đọc lại `v_kl_dashboard`** (không vá từng dòng ở client, số luôn khớp DB). **Dự phòng**: kênh rời `SUBSCRIBED` (`CHANNEL_ERROR`/`TIMED_OUT`/`CLOSED`) → làm mới mỗi 60 giây và chỉ báo vàng "Mất kết nối trực tiếp — đang làm mới mỗi 60 giây"; nối lại → chấm xanh "Cập nhật trực tiếp"; thêm làm mới khi tab quay lại foreground và khi có mạng; lúc mở màn hình chỉ báo "Đang kết nối…" (xám), chỉ sang cảnh báo vàng + polling nếu sau 4 giây vẫn chưa SUBSCRIBED (chủ dự án soi diff #46: bản đầu nháy vàng mỗi lần vào màn hình). Tên lớp chỉ báo khai báo nguyên văn (`LOP_CHE_DO`) và e2e kiểm màu tính toán trên bản build — bản đầu ghép chuỗi nên Tailwind cắt mất, dev server không lộ. Kênh bật khi mở màn hình KL (danh sách hoặc dashboard), chỉ đọc lại màn hình đang hiện, tắt khi đăng xuất. Không đăng ký `phu_trach_phong` (đổi phạm vi PCVP hiếm — cập nhật ở lần đọc lại kế tiếp). Ngăn chi tiết đang mở giữ nguyên (mở lại) sau mỗi lần vẽ lại.
- Test: e2e kịch bản 11 `kl-realtime.spec.js` 2 case × 2 kích thước (DB thêm/đổi tiến độ → ô số và dòng tự đổi không bấm gì; `setOffline` → chỉ báo dự phòng, có mạng lại → trực tiếp), local 4/4. Kiểm tra tay local: đổi ghi chú bằng CLI → dấu "tính đến" đổi sau ~1 giây.
- **PR 10F (2026-09-15)** — **form thêm nhiệm vụ KL mới** cho người có `quan_tri_kl` (`views/shared/kl/them-modal.js`; nút "Thêm nhiệm vụ" chỉ hiện với cờ, policy INSERT 0016 là chốt; thiết kế 5.4 + 6.8): chọn hội nghị có sẵn hoặc **hội nghị mới** (số, số KL — trùng báo "đã có", ngày ban hành ≤ hôm nay); nội dung, **chủ trì bắt buộc** (danh bạ trừ tài khoản hệ thống), cơ quan trình, **ngành → lĩnh vực khoá theo ngành, bắt buộc** (việc mới không phát sinh "Chưa phân loại"), loại thời hạn (Ký ban hành: hạn tự tính = ngày BH + N từ `kl_cau_hinh`, ô khoá; Có hạn cụ thể: hạn hoặc "chưa xác định" + lý do), hạn ≥ ngày BH với "còn N ngày" tức thì, văn bản triển khai, ghi chú/lĩnh vực chi tiết. "Lưu, nhập tiếp" giữ hội nghị/ngành/lĩnh vực/cơ quan trình cho kết luận có nhiều việc (tình huống "20 nhiệm vụ một hội nghị"). `nguon = 'app'`, `tao_boi` = người nhập, lịch sử ghi dòng tạo. Hội nghị tạo trước, nhiệm vụ sau (hai lệnh; nhiệm vụ lỗi thì hội nghị vẫn còn để chọn lại — không sao).
- Test: e2e kịch bản 12 `kl-them-nhiem-vu.spec.js` 2 case × 2 kích thước (thiếu lĩnh vực bị chặn; hội nghị mới + đủ trường → NV-xxx xuất hiện, ô Tổng +1, `nguon = app`; Ký ban hành khoá hạn = BH + 10); kịch bản 9 thêm kiểm "chuyên viên không thấy nút Thêm". Local 4/4. Ảnh: `docs/anh-man-hinh/gd10/kl-modal-them-nhiem-vu.png`.
- **PR 10E — phát hành `v2.3.0` (2026-09-15)**: PR chỉ tài liệu (CHANGELOG, TRANG-THAI, tài liệu thiết kế: quyết định 3 → "chặn từ GĐ10", hàng 2/4 của 3.2 điều chỉnh, ghi chú triển khai GĐ10, PR 11B bỏ mục minh chứng) + tag. Nội dung phát hành = 10A #43 (0021), 10B #44, 10C #45, 10D #46, 10F #47. Quy trình: backup tay → tag `v2.3.0` trên đầu nhánh phát hành sau khi CI xanh → duyệt environment (0021 lên production, **xác nhận trong phiên**) → smoke → merge → kiểm tra bản live 3 vai + `quan_tri_kl`. Kết quả ghi ở dòng sau khi hoàn tất.
- **Sự cố #48 (2026-09-15)**: PR phát hành `v2.3.0` (#48, `be45313`) bị merge **trước** khi gắn tag → tag `v2.3.0` chưa tồn tại, `deploy-prod` chưa chạy, production vẫn `v2.2.0`, 0021 chưa áp. Theo bài học rc1 không tag lên commit cũ: làm lại bằng nhánh `feature/phat-hanh-v2.3.0-lan-2` (một commit chỉ tài liệu: `docs/xu-ly-su-co.md` #13 — nguyên nhân, cách xử lý, chốt chặn "KHÔNG BẤM MERGE cho tới khi tag `production` đã gắn"; CHANGELOG; TRANG-THAI), tag `v2.3.0` lên đầu nhánh mới sau CI xanh + backup + xác nhận, merge sau khi có tag `production`.
- **PR CI rút gọn (2026-09-15, sau v2.3.0)** — chuẩn bị giai đoạn nhiều PR tài liệu: `ci.yml` thêm job `phan-loai` (PR chỉ đổi `docs/**`, `*.md` → hai job chậm `Áp migration + lint schema`, `Kiểm thử RLS + e2e trên staging` thay bằng job rỗng cùng tên, ruleset không đổi; không chắc thì chạy đủ; gitleaks và frontend luôn chạy); bỏ bước RLS token thật khỏi job PR (RLS đã chạy cục bộ với migration của PR) và chuyển sang `deploy-staging.yml` job `kiem-thu-rls` sau `db push`, không chặn deploy; cache Chromium Playwright theo phiên bản trong lock file. Nhánh có `phat-hanh` trong tên luôn chạy đủ (commit gắn tag phải có 4 check thật cho `kiem-tra` của deploy-prod). PR chỉ đổi workflow nên tự nó chạy CI đầy đủ (phép thử). `docs/kien-truc.md` mục 2–3, 5 và README của `tests/rls`, `tests/e2e` cập nhật.

## 20. Định hướng lại theo phụ lục Công văn 1400-CV/VPTU (2026-09-16, sau v2.3.0)
- **Bối cảnh**: module "Kết luận BTVTU" (GĐ8–10) xây thành một module đặt cạnh hệ thống, trong khi thông báo/kết luận của Thường trực Tỉnh ủy chính là bước "văn bản đến → tạo nhiệm vụ" của luồng gốc. Từ nay xương sống là phụ lục 1400: "Hệ thống quản trị nhiệm vụ theo kết quả", nguyên tắc 1-1-1-1-3, 5 chức năng MVP, mô hình 4 cấp.
- **PR-0 #50** (CI rút gọn cho giai đoạn nhiều PR tài liệu) — mục 19 dòng cuối.
...
## 21. Giai đoạn 14 (v3) — Thực thể thống nhất theo phụ lục 1400 · phát hành `v3.0.0`
- **PR 14AB #52 (2026-09-16)** — `0022` additive (danh mục `dm_san_pham` 7 loại, `dm_cap` 6 cấp, 3 cột đơn vị, `cho_phep_tao_moi`, 3 khoá `kl_cau_hinh` Vàng 3 / Đỏ đặc biệt 3 / rà soát 30; nhiệm vụ thêm Owner tài khoản, sản phẩm, cấp nhận/cấp quyết định, ngày nhận văn bản + cờ ước tính (185 việc cũ = ngày ban hành, điền với trigger tắt, không sinh lịch sử), `nhiem_vu_cha`, `theo_1400`; trigger ngày nhận, cha–con, Owner khớp đơn vị); `0023` RENAME `kl_nhiem_vu→nhiem_vu`, `kl_hoi_nghi→van_ban_giao_viec`, `kl_lich_su/kl_chi_dao/kl_dinh_chinh`, `dm_co_quan_trinh→dm_don_vi`, 4 cột (`chu_tri_id→nguoi_theo_doi`, `co_quan_trinh_ma→owner_don_vi_ma`, `hoi_nghi_id→van_ban_id`, `ghi_hoan_thanh_luc→dong_luc`); `0024` hàm `trang_thai()` 4 mức XANH/VÀNG/ĐỎ/ĐỎ ĐẶC BIỆT + lead time, view `v_nhiem_vu`, **bí danh** `kl_*`/`dm_co_quan_trinh`/wrapper `kl_trang_thai` cho mã cũ. RLS 132 (19 mới), bộ số 14/9 giữ.
- **PR 14C #53** — `0025`: 5 dòng phòng vào `dm_don_vi`; `kl_pham_vi` thêm nhánh Owner (A3 thấy việc mình là Owner/theo dõi, A2 phòng, PCVP theo phòng Owner); policy/guard cho Owner sửa sản phẩm/cấp/ngày nhận; **`giao_viec(p jsonb)`** (GV-3 theo vai, 1-1-1 với `theo_1400`, người theo dõi trong phạm vi người giao), **`xac_nhan_nhan_viec()`** chỉ ghi lịch sử (CN-2.2), văn bản không cần số hội nghị trừ KL_BTV. Frontend: form giao việc thống nhất (văn bản → Owner → sản phẩm → ngày nhận → hạn → cấp nhận → ngành/lĩnh vực → người theo dõi), danh sách "Nhiệm vụ" có cột Chịu trách nhiệm/Sản phẩm/chấm màu 4 mức, nút xác nhận nhận việc; module KL đọc tên thật. RLS 140, e2e 45.
- **PR 14D #54** — màn hình A1/A2/A3 chuyển sang `v_nhiem_vu`, bỏ luồng `tasks` ở frontend (dashboard ngoại lệ cũ, form giao việc A1/A2, theo dõi/duyệt, nộp minh chứng URL, luồng ý kiến `task_directives`, phân công lại); A1 mặc định Tổng quan, cây cán bộ + KPI phòng đánh giá theo Owner (kể cả Owner là chính phòng), việc theo dõi ghi riêng (CH-2); A2/A3 mặc định màn hình Nhiệm vụ (A3 lọc "việc của tôi"); modal bắt buộc xác nhận nhận việc chỉ với việc theo 1400, 185 việc cũ chỉ chip + nút tuỳ chọn; `state.nhiemVu`; e2e viết lại `nhiem-vu.spec` (bỏ kịch bản duyệt), bỏ case ý kiến theo `tasks` (→ GĐ18); unit 14.
- **Phát hành `v3.0.0` (PR chỉ tài liệu + tag)**: **migration 0022–0025 lên production** (backup tay trước, xác nhận trong phiên, tag trước merge); bảng `tasks`/`task_evidences`/`task_directives`, hàm luồng cũ và **bí danh `kl_*` giữ tới 18A** (drop sau một phát hành theo nếp additive → destructive). Kiểm tra sau phát hành: 3 vai + `quan_tri_kl` trên bản live; đối chiếu 185 / 134 lĩnh vực NULL / 78 thiếu minh chứng và bộ số 146/16/8/6/6/3 tại 14/9 (một kỳ đối chiếu sheet, CH-15).

## 22. Giai đoạn 15 (v3) — Điều hành ngoại lệ (GĐ17 + 18B của lộ trình, đảo lên theo phụ lục 1400 bước 5) · phát hành `v3.1.0`
- **PR 15A #56 (2026-09-16)** — `0026_dieu_hanh_ngoai_le.sql`: `chi_dao` thêm loại `Y_KIEN` (tạo với `DA_PHAN_HOI`, không tính "chờ phản hồi") và `PHAN_HOI` (`tra_loi_cho` = chỉ đạo gốc), bảng `chi_dao_da_doc`, **INSERT trực tiếp `chi_dao` bị thu quyền** — chỉ qua hàm `chi_dao_gui / chi_dao_phan_hoi / chi_dao_dong` (A1/A2 trong phạm vi; GIA_HAN chỉ A1/`quan_tri_kl`, A2 chỉ việc mình giao không từ KL/TB, đổi `han_xu_ly` + `so_lan_gia_han` theo DL-5, việc "Ký ban hành" chuyển sang "Có hạn cụ thể"; GIAO_LAI trong phạm vi người ra chỉ đạo); `nguoi_lien_quan()` (Owner, người theo dõi, trưởng phòng, PCVP phụ trách/kiêm nhiệm, Chánh VP, người trong luồng — mỗi người một tin, trừ người gửi); `direct_messages` thêm `loai = he_thong` + `nhiem_vu_id`; `dat_cap_quyet_dinh()`; view `v_ngoai_le` (chỉ Đỏ/Đỏ đặc biệt, 4 trường CN-5.2, nhóm đang tra soát). RLS 149 (9 mới).
- **PR 15B #57** — thanh bên "Dashboard" (A1; A2 phạm vi phòng): hàng 1 bảng ngoại lệ từ `v_ngoai_le` với nút Chỉ đạo và ô cấp cần quyết định tại chỗ, hàng 2 tình hình chung gộp ô nhóm, hàng 3 theo Owner (đơn vị) + người theo dõi ghi riêng (CH-2); khối "Chỉ đạo" dạng luồng bình luận trên mọi nhiệm vụ (gửi/phản hồi/đóng qua hàm 0026, realtime, đã đọc khi mở); chuông thông báo tin hệ thống ở thanh trên cho mọi vai (toast realtime, bấm mở đúng nhiệm vụ); chat 1-1 chỉ tin loại người; e2e `dieu-hanh.spec` (A1 → A3 → A1, cấp quyết định), 40 test.
- **CI e2e 5m20s → 3m25s** (cùng PR #57): `kl-realtime` nghe `window` offline/online (dự phòng ngay, mở kênh mới khi có mạng — kịch bản 11 chờ ≤ 5 giây thay vì 150 giây); project mobile chỉ chạy 4 spec nhạy bố cục (đăng nhập, chuyên viên, form giao việc, nhận việc). Việc còn lại để rút thêm: 2 worker Playwright.
- **Phát hành `v3.1.0` (PR chỉ tài liệu + tag)**: **migration 0026 lên production** (backup tay trước, xác nhận trong phiên, tag trước merge). Sau phát hành: kiểm 3 vai trên bản live (Dashboard, chỉ đạo, chuông), đối chiếu 185 / 134 / 78 và 146/16/8/6/6/3. Lộ trình đã đảo: GĐ16 minh chứng có cấu trúc, GĐ17 cảnh báo tự động (cron GitHub), GĐ18 vai trò A0 + dọn luồng cũ.

## 23. Giai đoạn 15 — PR 15C giao diện điều hành · phát hành `v3.1.1` (chỉ frontend)
- **PR 15C #59 (2026-09-16)** — ngăn chi tiết nhiệm vụ đảo thứ tự: khối "Chỉ đạo" (ô chọn loại + nội dung + Gửi; vai không ra chỉ đạo thấy ô phản hồi cùng vị trí) lên đầu ngay dưới tiêu đề, bảng chi tiết gói vào khối gập "Xem chi tiết"; Dashboard/chuông mở thẳng vào ô nhập; tên + chức vụ người dùng lên đầu thanh bên dưới logo, thanh bên dính, 360px tên ở thanh trên. Không đổi hàm/RPC. e2e dieu-hanh + kl-chuyen-vien cập nhật.
- **PR 15D #61** — migration `0027_sua_ma_nhiem_vu.sql`: mặc định `nhiem_vu.ma` của 0014 dùng `lpad(…, 3)` nên CẮT số khi sequence vượt 999 (`NV-1002` → `NV-100` trùng khoá; staging đã tới 1001 vì mỗi lần test tiêu một số, production 185) → hàm `nhiem_vu_ma_moi()` đệm tối thiểu 3 chữ số không cắt; test RLS `kl-0027` đặt sequence 999 qua `supabase db query` → `NV-1000`, `NV-1001` (RLS 150). Kèm: kịch bản 11 chờ nối lại tới 60 giây (runner CI nối socket theo backoff), phát hiện mất mạng vẫn ≤ 5 giây.
- **Phát hành `v3.1.1`** (PR chỉ tài liệu + tag): **migration 0027 lên production** (backup tay trước, xác nhận trong phiên, tag trước merge) + giao diện điều hành #59. Gộp vào một đợt để không tách phát hành.

## 24. Giai đoạn 16 (v3) — Minh chứng có cấu trúc và đóng nhiệm vụ (MC-2…MC-6, DL-4; CH-6 = B) · phát hành `v3.2.0` (**có migration 0028**)
- **PR 15E #62 (2026-09-16, chỉ frontend)** — tách nút "Chi tiết" (mở ngăn, bảng thông tin mở sẵn) và "Chỉ đạo" (A1/A2) / "Phản hồi" (Owner, người theo dõi) trên danh sách Nhiệm vụ và bảng ngoại lệ; 360px hai nút xếp dọc; vẽ lại realtime giữ đúng chế độ ngăn đang mở.
- **CI-4 #63** — e2e mỗi context một phiên đăng nhập riêng (storageState theo vai), `workers = 1` (thử 2 worker rồi quay về vì 3 tài khoản dùng chung).
- **PR 16A #64** — migration `0028_minh_chung.sql`: bảng `minh_chung` (`so_hieu` | `chu_cu` | `tep` để sẵn NULL, không Storage), RLS SELECT theo phạm vi nhiệm vụ, ghi chỉ qua hàm; **một vị từ hợp lệ** `minh_chung_la_hop_le()` = đã nộp đủ ba trường và chưa bị bác, dùng chung cho `dong_nhiem_vu()`, trigger 0021 và `v_nhiem_vu.so_minh_chung_hop_le`; chuyển 76 minh chứng chữ cũ thành dòng `chu_cu` (giữ nguyên văn, tách số hiệu/ngày, không coi là vi phạm); `thieu_minh_chung` từ cột generated thành cột thường do trigger tính lại theo chữ + bảng (việc `theo_1400` chỉ nhận bảng); hàm `nop_minh_chung(p)` (Owner/người theo dõi, ngày văn bản ∈ [ngày ban hành hoặc ngày nhận, hôm nay giờ Việt Nam]), `xac_nhan_minh_chung()` (người theo dõi, A1/A2 trong phạm vi; không tự xác nhận của mình; bác phải có lý do), `dong_nhiem_vu()` (ngày hoàn thành mặc định = ngày văn bản hợp lệ mới nhất, chốt lead time); mỗi hành động ghi `lich_su` + tin hệ thống. Sửa trong PR: `current_setting()` của GUC chưa set trả NULL làm điều kiện RAISE im lặng → bọc `coalesce`; tính lại cờ không ghi đè `cap_nhat_luc/cap_nhat_boi`. Test RLS `kl-0028` 10 test (vai, phạm vi, chữ cũ, lead time, cận dưới NULL, múi giờ).
- **PR 16B #65 (chỉ frontend)** — khối "Minh chứng" trong ngăn chi tiết (danh sách, nhãn "Minh chứng cũ", Xác nhận hợp lệ / Không hợp lệ kèm lý do); hộp "Nộp minh chứng" ba ô bắt buộc thay ô chữ tự do; nút "Đóng nhiệm vụ" chỉ sáng khi `so_minh_chung_hop_le > 0`, hộp ngày hoàn thành gợi ý; lead time trong ngăn; modal Cập nhật không cho việc `theo_1400` chọn Hoàn thành; ô Dashboard "chưa có minh chứng" đọc `thieu_minh_chung` từ view. e2e `kl-minh-chung.spec` 3 test; kịch bản 5 `nhiem-vu.spec` đổi theo luồng mới.
- **Phát hành `v3.2.0`** (PR chỉ tài liệu + tag): **migration 0028 lên production** (backup tay trước, xác nhận trong phiên, tag trước merge) + #62, #63, #65. Chưa vào kênh realtime: bảng `minh_chung` (người khác nộp/bác thì phiên đang mở thấy khi nạp lại).

## 25. Giai đoạn 17 + 18 (v3) — Cảnh báo tự động 3 cấp, vai trò A0 Thường trực, dọn luồng cũ · phát hành `v3.4.0` (**có migration 0029–0031**; `v3.3.0` của lộ trình gộp vào đây, không phát hành riêng)
- **GĐ17 #67 (2026-09-16)** — migration `0029_canh_bao.sql`: bảng `canh_bao` (sổ gửi, chống trùng theo ngày Việt Nam), `canh_bao_nguoi_nhan()` 3 cấp (VÀNG → Owner/người theo dõi; ĐỎ → + trưởng phòng / lãnh đạo đơn vị ngoài; ĐỎ ĐẶC BIỆT → + PCVP phụ trách, Chánh VP), `canh_bao_quet()` idempotent chỉ service_role gọi; workflow `canh-bao.yml` cron 07:30 giờ Việt Nam (KT-4, không `pg_cron`), tin hệ thống qua chuông GĐ15; chạy thử trên staging OK; test RLS + mốc ngày 17–23h UTC.
- **GĐ18 #68 (2026-09-16)** — migration `0030_vai_tro_a0.sql`: vai trò `A0` Thường trực Tỉnh ủy (CH-11 = A) xem toàn bộ nhiệm vụ/ngoại lệ/chỉ đạo/minh chứng/cảnh báo, chỉ ghi chỉ đạo loại "Ý kiến"; mọi hàm ghi và policy nhắn tin loại A0 tường minh; A0 chỉ nhận tin phản hồi vào luồng ý kiến mình mở; frontend: vào thẳng Dashboard lọc sẵn Đỏ đặc biệt, ẩn mọi nút ghi khác. Migration `0031_bo_luong_cu.sql` — **DESTRUCTIVE, không quay lui bằng migration, khôi phục từ backup**: bỏ luồng `tasks`/`task_directives`/`task_evidences` v2, `view_exception_dashboard`, 11 hàm, 8 policy, bí danh `kl_*`/`v_kl_dashboard`/`kl_trang_thai` (chủ dự án xác nhận dữ liệu `tasks` production không cần giữ). Seed: `demo_a0` + tài khoản riêng từng spec e2e; Playwright `workers = 2`, bộ e2e ~2 phút; RLS 131 test, e2e 28 test.
- **Phát hành `v3.4.0`** (PR chỉ tài liệu + tag): **migration 0029–0031 lên production** (backup pg_dump tự động trước `db push` trong `deploy-prod.yml`; xác nhận trong phiên; tag gắn lên commit đầu nhánh release, KHÔNG merge trước khi tag) + #67, #68. Sau phát hành: bấm *Enable workflow* cho `canh-bao.yml` nếu GitHub tắt schedule; tạo 3 tài khoản A0 thật bằng script có vết (tài liệu chỉ ghi chức danh).

## 26. Giai đoạn 19 (v3) — Chỉ đạo Thường trực `CHI_DAO_TT` (CH-16) · phát hành `v3.5.0` (**có migration 0032**)
- **GĐ19 #70 (2026-09-16)** — migration `0032_chi_dao_thuong_truc.sql`: loại chỉ đạo `CHI_DAO_TT` chỉ A0 gửi; người nhận tự tính (A0 không chọn) = Chánh VP + PCVP phụ trách phòng Owner/theo dõi + lãnh đạo VP phụ trách đơn vị ngoài, khử trùng; hạn phản hồi mặc định 2 ngày làm việc (`ngay_lam_viec_sau`, khoá `kl_cau_hinh.chi_dao_tt_han_phan_hoi_ngay`, theo ngày Việt Nam); người nhận phản hồi hoặc chuyển thành chỉ đạo con (DON_DOC/GIA_HAN/… gắn `tra_loi_cho` = luồng TT, luồng TT sang "Đã phản hồi", A0 nhận tin); A0 đóng được luồng TT của mình (người nhận/quản trị không; Y_KIEN giữ như 0030); tin hệ thống của luồng TT chỉ tới người nhận + A0; quét quá hạn phản hồi trong `canh_bao_quet` (muc `CHI_DAO_TT`, cột `canh_bao.chi_dao_id`, nhắc lại theo `canh_bao_nhac_lai_ngay`, không leo thang); view `v_chi_dao_tt`. Frontend: Dashboard thêm khối "Chỉ đạo Thường trực" đầu trang (A1: chờ phản hồi của mình; A0: đã gửi + trạng thái phản hồi), ô nhập của A0 có hai lựa chọn Ý kiến/Chỉ đạo (+ hạn phản hồi tuỳ chọn), người nhận có ô phản hồi và ô "Chuyển thành chỉ đạo". Test: RLS `kl-0032` 7 test (138 pass), e2e `chi-dao-tt.spec` chạy ở project riêng sau `desktop`.
- **Phát hành `v3.5.0`** (PR chỉ tài liệu + tag): **migration 0032 lên production** (backup tự động trước `db push`; xác nhận trong phiên; tag gắn lên commit đầu nhánh release, KHÔNG merge trước khi tag) + #70.
- **Hoãn sang `v3.6`** (ngoài 5 bước MVP 1400): xuất HTML/PDF hai bản; quản trị nhân sự hiệu lực theo ngày; nhập bổ sung dữ liệu chuyển đổi (CH-13); đặc tả nối V-Office. **Việc nhỏ để sau**: cộng số chỉ đạo TT chờ phản hồi vào ô "cấp cần quyết định" (hiện đếm ở tiêu đề khối); `so_chi_dao_cho_phan_hoi` trong `v_nhiem_vu` tách riêng CHI_DAO_TT (dòng của A3 đang đếm cả TT).

## 27. Giai đoạn 20–24 (v3) — Giao diện v7, Trung tâm điều hành A0, độ khẩn, giao thay mặt, từ chối có duyệt, quản trị hệ thống · phát hành `v3.6.0` (**migration 0033–0044 đã trên production**)
- **GĐ20 #73** (0033): giao diện v7 theo mockup, A0 Trung tâm điều hành, `v_ngoai_le.khau`. **GĐ21 #74** (0034): từ chối nhận việc có duyệt, A0 Cán bộ + Nhắn tin. **GĐ22 #75** (0035–0037): độ khẩn 4 cấp, giao thay mặt, Thường trực giao việc, diễn biến tại chỗ, công tắc kiểm thử `KIEM_THU_MOI_TRUONG`.
- **GĐ23 #76** (0038–0042): dải nhận diện + menu bánh răng theo vai, đổi mật khẩu lần đầu, Edge Function `quan-tri-tai-khoan`, Dọn dữ liệu hai bước, mốc backup, nạp lại đúng việc sau mọi hành động ghi. **GĐ24 #77–#79** (0043–0044): hộp thả xuống chuông/bánh răng, dải "Cần xử lý ngay" có nút hành động, bộ `E2E-SEED` + phòng thử `E2E_PT`, kiểm phạm vi việc mẫu ở beforeAll.
- **Phát hành `v3.6.0`** (PR tài liệu `release/v3.6.0` + tag trên commit đầu nhánh, không merge trước tag): production DB đã ở `0044` (CI kiểm thử trên production đã `db push` từng PR) → deploy chỉ frontend + Edge Function; backup trước phát hành `prod-20260917-1605-truoc-v3.6.0`.
- ~~Lỗi đã biết: bước "Ghi mốc backup" trong `backup-dinh-ky.yml` thoát mã 127~~ — **đã sửa ở `v3.6.1`** (#82).

## 28. Sửa sau kiểm tra v3.6.0 · phát hành `v3.6.1` (**không migration**)
- **#81 (2026-09-17)** — nhãn độ khẩn trên thẻ việc nghẽn A0/A1 bị kéo giãn: `components.css` `.the .ten > span`, `layout.css` khoá `.ten b/span` của dải nhận diện vào `.dai .ten` (mô tả thẻ hiện lại trên điện thoại). Nhãn thanh trái khâu `CHO_MINH_CHUNG` → "Việc Đỏ chờ xác nhận minh chứng" (chỉ đếm trong việc Đỏ; ô số đếm mọi minh chứng chờ). **Việc mới chờ xác nhận theo TỪNG NGƯỜI**: cờ `toi_da_xac_nhan` — chủ trì và người theo dõi mỗi người tự bấm nhận (dải Cần xử lý ngay, khối Thường trực giao, thẻ Việc mới giao A3, nút ngăn chi tiết; khớp `kl_so_chua_xu_ly.viec_moi`); `da_nhan_viec`/`v_ngoai_le`/cảnh báo giữ "owner HOẶC người theo dõi". Spec mới `viec-moi-tung-nguoi` (owner xác nhận bằng RPC dưới token của mình; số trên nút = số dòng = RPC), `dieu-hanh` kiểm nhãn `.dk` gọn; lib e2e `clientCuaVai`.
- **#82 (2026-09-17)** — `backup-dinh-ky.yml`: hai khoá `retention-days`/`if-no-files-found` của `upload-artifact` lạc vào khối `run:` của bước "Ghi mốc backup" (exit 127) — đặt lại dưới `with:`; kiểm chứng bằng run *Backup định kỳ production* #7 xanh.
- **Phát hành `v3.6.1`** (2026-09-18, PR chỉ tài liệu `release/v3.6.1` + tag trên commit đầu nhánh, không merge trước tag; không migration → deploy chỉ frontend + Edge Function).

## 29. Sửa sau kiểm tra v3.6.2 — Giao lại đổi chủ trì; nạp lại số đếm sau khi duyệt từ chối (**có migration 0045**)
- **0045** `chi_dao_gui` GIAO_LAI: đổi **chủ trì** (`chu_tri_moi`, bắt buộc; `nguoi_theo_doi_moi` tuỳ chọn), `owner_don_vi_ma` theo phòng chủ trì mới, `cap_nhan_san_pham` tính lại bằng hàm dùng chung `kl_cap_nhan_mac_dinh` (giao_viec gọi lại, không chép logic); chặn việc hoàn thành/đã đóng; minh chứng cũ giữ; lịch sử `giao_lai` "Chuyển chủ trì từ A sang B" (không ghi lý do từ chối); cũ và mới nhận tin; cờ bị từ chối tự xoá; chủ trì mới xác nhận nhận việc lại.
- Frontend: ô Giao lại (thẻ điều hành, dải Cần xử lý ngay, ngăn việc, ngăn chi tiết) chọn chủ trì mới + người theo dõi gợi ý theo cấp quản lý của chủ trì mới (`goiYTheoDoi` của biểu mẫu giao việc), sửa được; nhãn/thông báo đổi sang "chủ trì". Mọi đường ghi (duyệt từ chối, giao lại, đôn đốc, phản hồi, xác nhận nhận, nộp/xác nhận minh chứng) gọi `lamMoiHuyHieu()` → số trên dải và huy hiệu đổi ngay.
- Seed: `demo_e2e_cv2` (A3, `E2E_RT`) + đơn vị `dm_don_vi` `E2E_RT`; nằm trong diện xoá của bộ "dữ liệu thử" (0043/0044). RLS `kl-0026`/`kl-0034` theo tham số mới; e2e mới `giao-lai-chu-tri` (gd22).

## 30. Đặc tả giao diện v8 (chỉ tài liệu, không mã, không migration)
- **2026-09-21** — thêm `docs/ui-ux/mockup-v8/`: `DESIGN-V8.md` (token màu/chữ, khung chung, 7 màn hình) + 7 file HTML tĩnh (đăng nhập, A0 Trung tâm điều hành, A1/A2/A3 hôm nay, tab Nhiệm vụ, tab Giao việc) + `assets/` (ảnh Bản Giốc 380KB, biểu trưng). Đây là đặc tả UI/UX đã chốt, thay `mockup-v7.html` (giữ làm lịch sử).
- `SPEC.md` (đầu file + mục 6), `DESIGN.md` (đầu file + mục 9), `TRANG-THAI.md` (đầu file, Đọc trước, Kế tiếp) trỏ về `mockup-v8/`; `DESIGN-V8.md` thắng `DESIGN.md` khi khác nhau. Triển khai giao diện v8 là việc kế tiếp, tách PR theo màn hình.

## 31. Giao diện v8 đợt 1 — khung chung: token, thanh đầu trang, menu dọc (chỉ frontend + e2e, không migration)
- **Token** (`tokens.css`): giá trị theo bảng DESIGN-V8 (đỏ B3121B, navy 1F3A5F, vàng E8B923, nền trang F6F4EF, viền E3DED4, chữ 1B1B1B/5B6270, bộ Xanh/Vàng/Đỏ), giữ TÊN biến v7 (`--lam` = navy); tiêu đề **Lora** tự chứa (`public/fonts/lora-*.woff2`, phông biến thiên 400–700), nội dung Be Vietnam Pro.
- **Thanh đầu trang 72px** (`dau-trang.css`, `index.html`): ảnh Bản Giốc bản 1200px `public/brand/ban-gioc-thanh-dau.jpg` (~110KB, ảnh tạm — `brand/README.md`) mờ 22% + lớp phủ navy→đỏ + dải núi SVG + vệt vàng; biểu trưng, dòng cơ quan, tên hệ thống in hoa; ô tìm 280px luôn mở (`tim-nhanh.js`), chuông huy hiệu vàng, tên + vai, bánh răng. Bỏ cụm cờ SVG.
- **Menu dọc 236px** (`menu-doc.css`, `nav.js`, `menu.js` thêm `nhom`): nhóm Điều hành / Theo dõi / Trao đổi / Hệ thống, mục chọn nền FBE9EA chữ đỏ, chân menu đọc `phien-ban.json`; 601–900px thu thành thanh biểu tượng dính; ≤ 600px thanh 56px dính + ngăn kéo (`#menuMoBtn`, `#menuNen`, Esc), thanh dưới 3 mục giữ nguyên.
- Thẻ/nút/ô nhập (`components.css`): nút 32px bo 8px, ô nhập 42px, thẻ bo 12px viền E3DED4. Nội dung từng màn hình giữ nguyên (đợt 2). e2e `bo-cuc-mobile` neo theo `#menuMoBtn`/`.ten b`, thanh đầu trang 56/72px, menu `sticky`. Ảnh: `docs/anh-man-hinh/v8-dot-1/`.

...
test.describe.serial('Luồng giao việc → tiếp nhận → nộp minh chứng → duyệt', () => {
  ...
  test('Kịch bản 4: A2 giao việc cho chuyên viên trong phòng', async ({ page }) => {
    ...
    await expect(page.locator('#toastContainer')).toContainText('Đã phát hành giao việc cho cán bộ.');
  ...
  test('Kịch bản 5: A3 bắt buộc tiếp nhận rồi nộp minh chứng', async ({ page }) => {
    ...
    await expect(page.locator('#toastContainer')).toContainText('Đã tiếp nhận nhiệm vụ.');
  ...
  test('Kịch bản 6: A2 duyệt minh chứng → nhiệm vụ hoàn thành', async ({ page }) => {
    ...
    await approvalRow.getByRole('button', { name: 'Duyệt hoàn thành' }).click();
    await expect(page.locator('#toastContainer')).toContainText('Nhiệm vụ hoàn thành.');

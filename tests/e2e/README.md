...
- Chạy trong CI ở job `Kiểm thử RLS + e2e trên staging` (`ci.yml`, mọi PR trừ PR chỉ tài liệu) — 8 lượt đăng nhập; Chromium được cache theo phiên bản Playwright; `tests/rls` token thật đã chuyển sang `deploy-staging.yml`; xem `docs/kien-truc.md`.

...
async function coMigration0021() {
  ...
  const { data: hn } = await db.from('kl_hoi_nghi').select('id').order('ngay_ban_hanh').limit(1).maybeSingle();
  if (!hn) return false;
  const r = await db.from('kl_nhiem_vu').insert({ hoi_nghi_id: hn.id, chu_tri_id: IDS.cv1, ma: 'NV-T29', noi_dung: 'KL-MC dò 0021',
  ...
  await db.from('kl_nhiem_vu').delete().in('id', r.data.map((x) => x.id));
  return false;
}
const SKIP = (await coMigration0021()) ? false : 'Chưa có migration 0021 (hoặc 0014–0016) trên project này (chạy lại sau khi merge).';

const capNhat = async (username, ma, patch) => (await userClient(username)).from('kl_nhiem_vu').update(patch).eq('id', id[ma]).select('tien_do_ma, thieu_minh_chung, ghi_hoan_thanh_luc');

describe('0021 — Hoàn thành bắt buộc minh chứng và ngày hoàn thành', { skip: SKIP }, () => {
  before(async () => {
    fx = await setupKlFixtures();
    const db = adminClient();
    await db.from('kl_nhiem_vu').delete().in('ma', MA);
    const base = { hoi_nghi_id: fx.hn, chu_tri_id: IDS.cv1, nganh_ma: 'KINH_TE_TONG_HOP', loai_thoi_han_ma: 'CO_HAN_CU_THE', han_xu_ly: '2026-10-31' };
  ...
  after(async () => {
    await adminClient().from('kl_nhiem_vu').delete().in('ma', MA);
  ...
  test('3. đủ minh chứng + ngày → Hoàn thành; ghi lúc chuyển; lịch sử ghi ba cột với người sửa = chủ trì', async () => {
    ...
    assert.equal(r.data[0].thieu_minh_chung, false); assert.ok(r.data[0].ghi_hoan_thanh_luc);
    const { data: ls } = await adminClient().from('kl_lich_su').select('cot, nguoi_sua, nguon').eq('nhiem_vu_id', id['NV-T21']).neq('cot', '*').order('cot');
  ...
  test('7–8. INSERT: nguon = app Hoàn thành thiếu minh chứng → chặn (kể cả service_role); nguon = excel → nhập nguyên trạng, cờ thieu_minh_chung', async () => {
    const db = adminClient();
    const base = { hoi_nghi_id: fx.hn, chu_tri_id: IDS.cv1, loai_thoi_han_ma: 'CHO_QUYET_DINH', tien_do_ma: 'HOAN_THANH', ngay_hoan_thanh: '2026-09-01' };
    const app = await db.from('kl_nhiem_vu').insert({ ...base, ma: 'NV-T25', noi_dung: 'KL-MC app thiếu' }).select('id');
    assert.match(app.error?.message || '', /phải có minh chứng/);
    const xl = await db.from('kl_nhiem_vu').insert({ ...base, ma: 'NV-T25', noi_dung: 'KL-MC excel thiếu', nguon: 'excel', ngay_hoan_thanh: null }).select('id, thieu_minh_chung');
    assertOk(xl, 'excel nhập nguyên trạng'); assert.equal(xl.data[0].thieu_minh_chung, true);
    await db.from('kl_nhiem_vu').delete().eq('id', xl.data[0].id);
  });
  test('9. đính chính tien_do_ma → Hoàn thành trên dòng không minh chứng: duyệt bị chặn tại trigger, đề nghị vẫn chờ duyệt', async () => {
    ...
    const { data } = await adminClient().from('kl_dinh_chinh').select('trang_thai').eq('id', dn.data).single();

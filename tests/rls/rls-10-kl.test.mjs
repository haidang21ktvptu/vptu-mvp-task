...
async function soThay(username) {
  ...
  const r = await c.from('kl_nhiem_vu').select('id').like('noi_dung', 'RLS-TEST%');
  assertOk(r, `${username} select`);
  return r.data.length;
}

describe('RLS-10 phạm vi đọc kl_nhiem_vu (quyết định 7)', { skip: SKIP }, () => {
  after(donPhanCong);
  ...
  test('Hội nghị: thấy khi có nhiệm vụ trong phạm vi; danh mục và cấu hình ai cũng đọc; v_kl_dashboard lọc theo RLS', async () => {
    ...
    const hn = await cv1.from('kl_hoi_nghi').select('id').eq('id', fx.hn);
    assertOk(hn, 'cv1 hội nghị'); assert.equal(hn.data.length, 1);
    const qtht = await userClient('demo_qtht');
    const hn0 = await qtht.from('kl_hoi_nghi').select('id').eq('id', fx.hn);
  ...
  test('bị chặn: anon mọi bảng KL và view', async () => {
    for (const t of ['kl_nhiem_vu', 'kl_hoi_nghi', 'kl_lich_su', 'kl_chi_dao', 'kl_dinh_chinh', 'kl_cau_hinh', 'dm_nganh', 'v_kl_dashboard']) {
...
describe('RLS-10 ghi: chủ trì, quan_tri_kl, chỉ đạo, lịch sử, đính chính', { skip: SKIP }, () => {
  ...
  test('chủ trì sửa tiến độ/minh chứng việc mình được; sửa nội dung/chủ trì bị chặn (guard); việc người khác 0 dòng', async () => {
    const cv1 = await userClient('demo_cv1');
    const ok = await cv1.from('kl_nhiem_vu').update({ minh_chung: 'RLS-TEST minh chứng', van_ban_trien_khai: 'KH 01' }).eq('id', fx.n1).select('id');
    assertOk(ok, 'sửa minh chứng'); assert.equal(ok.data.length, 1);
    assertDenied(await cv1.from('kl_nhiem_vu').update({ noi_dung: 'đổi' }).eq('id', fx.n1).select('id'), 'đổi nội dung');
    assertDenied(await cv1.from('kl_nhiem_vu').update({ chu_tri_id: IDS.cv2 }).eq('id', fx.n1).select('id'), 'đổi chủ trì');
    assertNoRows(await cv1.from('kl_nhiem_vu').update({ minh_chung: 'x' }).eq('id', fx.n4).select('id'), 'việc người khác');
    assertDenied(await cv1.from('kl_nhiem_vu').delete().eq('id', fx.n1).select('id'), 'xoá');
  });
  test('bị chặn: A3/A2/Chánh VP không có quan_tri_kl thêm nhiệm vụ, hội nghị; sửa cấu hình', async () => {
    const row = { hoi_nghi_id: fx.hn, chu_tri_id: IDS.cv1, noi_dung: 'RLS-TEST mới', loai_thoi_han_ma: 'CHO_QUYET_DINH' };
    for (const u of ['demo_cv1', 'demo_truongphong', 'demo_cvp']) {
      const c = await userClient(u);
      assertDenied(await c.from('kl_nhiem_vu').insert(row).select('id'), `${u} insert nhiệm vụ`);
      assertDenied(await c.from('kl_hoi_nghi').insert({ so_hoi_nghi: 998, so_ket_luan: 'RLS-TEST', ngay_ban_hanh: '2026-08-01' }).select('id'), `${u} insert hội nghị`);
  ...
  test('quan_tri_kl (cấp tạm cho cv2): thấy tất cả, thêm hội nghị + nhiệm vụ, sửa mọi cột; thu cờ → mất ngay', async () => {
    ...
    const hn = await cv2.from('kl_hoi_nghi').insert({ so_hoi_nghi: 998, so_ket_luan: 'RLS-TEST', ngay_ban_hanh: '2026-08-01' }).select('id').single();
    assertOk(hn, 'quan_tri_kl thêm hội nghị');
    const nv = await cv2.from('kl_nhiem_vu').insert({ hoi_nghi_id: hn.data.id, chu_tri_id: IDS.cv1, noi_dung: 'RLS-TEST N8 mới', loai_thoi_han_ma: 'CHO_QUYET_DINH' }).select('id, ma, tao_boi').single();
    assertOk(nv, 'quan_tri_kl thêm nhiệm vụ'); assert.match(nv.data.ma, /^NV-\d{3}$/); assert.equal(nv.data.tao_boi, IDS.cv2);
    assertOk(await cv2.from('kl_nhiem_vu').update({ noi_dung: 'RLS-TEST N8 đã sửa', nganh_ma: 'NOI_CHINH' }).eq('id', nv.data.id).select('id'), 'sửa mọi cột');
  ...
  test('chỉ đạo: A2 phòng mình thêm được; A3 chủ trì và PCVP ngoài phạm vi bị chặn; đọc theo phạm vi; sửa/xoá trực tiếp bị chặn', async () => {
    ...
    assertOk(await tp.from('kl_chi_dao').insert({ nhiem_vu_id: fx.n1, nguoi_gui: IDS.truongphong, loai: 'YEU_CAU_MINH_CHUNG', noi_dung: 'RLS-TEST D2' }).select('id'), 'A2 chỉ đạo');
    assertDenied(await tp.from('kl_chi_dao').insert({ nhiem_vu_id: fx.n1, nguoi_gui: IDS.cvp, loai: 'DON_DOC', noi_dung: 'giả' }).select('id'), 'giả nguoi_gui');
    const cv1 = await userClient('demo_cv1');
    assertDenied(await cv1.from('kl_chi_dao').insert({ nhiem_vu_id: fx.n1, nguoi_gui: IDS.cv1, loai: 'DON_DOC', noi_dung: 'x' }).select('id'), 'A3 chỉ đạo');
    const pcvp2 = await userClient('demo_pcvp2');
    assertDenied(await pcvp2.from('kl_chi_dao').insert({ nhiem_vu_id: fx.n1, nguoi_gui: IDS.pcvp2, loai: 'DON_DOC', noi_dung: 'x' }).select('id'), 'PCVP ngoài phạm vi');
    const r = await cv1.from('kl_chi_dao').select('id').eq('nhiem_vu_id', fx.n1);
    assertOk(r, 'chủ trì đọc chỉ đạo'); assert.equal(r.data.length, 2);
    const r2 = await pcvp2.from('kl_chi_dao').select('id').eq('nhiem_vu_id', fx.n1);
    assertOk(r2, 'ngoài phạm vi đọc'); assert.equal(r2.data.length, 0);
    assertDenied(await tp.from('kl_chi_dao').update({ trang_thai: 'DA_DONG' }).eq('id', fx.d1).select('id'), 'update trực tiếp');
    assertDenied(await tp.from('kl_chi_dao').delete().eq('id', fx.d1).select('id'), 'delete');
  });
  test('lịch sử: chủ trì/A2 đọc được dòng của việc trong phạm vi; ngoài phạm vi 0 dòng; ghi trực tiếp bị chặn', async () => {
    const cv1 = await userClient('demo_cv1');
    const r = await cv1.from('kl_lich_su').select('cot').eq('nhiem_vu_id', fx.n1);
    assertOk(r, 'cv1 lịch sử'); assert.ok(r.data.length >= 2 && r.data.some((l) => l.cot === 'minh_chung'));
    const cv2 = await userClient('demo_cv2');
    const r0 = await cv2.from('kl_lich_su').select('cot').eq('nhiem_vu_id', fx.n1);
    assertOk(r0, 'cv2 lịch sử'); assert.equal(r0.data.length, 0);
    assertDenied(await cv1.from('kl_lich_su').insert({ nhiem_vu_id: fx.n1, cot: 'x', gia_tri_moi: 'giả' }).select('id'), 'insert lịch sử');
    assertDenied(await cv1.from('kl_lich_su').delete().eq('nhiem_vu_id', fx.n1).select('id'), 'delete lịch sử');
  });
  test('đính chính: chủ trì đề nghị → nhom_dem DANG_DINH_CHINH; người đề nghị tự duyệt bị chặn; quan_tri_kl duyệt → cột đổi, lịch sử nguon dinh_chinh', async () => {
    const cv1 = await userClient('demo_cv1');
    assertDenied(await cv1.from('kl_dinh_chinh').insert({ nhiem_vu_id: fx.n1, de_nghi_boi: IDS.cv1, cot: 'han_xu_ly', gia_tri_moi: '2026-10-30', ly_do: 'x' }).select('id'), 'insert trực tiếp');
    ...
    const { data: nv } = await adminClient().from('kl_nhiem_vu').select('han_xu_ly').eq('id', fx.n1).single();
    assert.equal(nv.han_xu_ly, '2026-10-30');
    const { data: ls } = await adminClient().from('kl_lich_su').select('cot, gia_tri_moi, nguon, dinh_chinh_id').eq('nhiem_vu_id', fx.n1).eq('nguon', 'dinh_chinh');

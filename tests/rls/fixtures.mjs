...
//   T3: phòng TONG_HOP, giao thẳng bởi PCVP — assigned demo_cv1, leader/creator demo_pcvp (A2 không phải party)
...
async function createFixtures() {
  ...
  const { data: tasks, error: e1 } = await db.from('tasks').insert([
    ...
    {
      title: `${TAG} T3`, resolution_code: TAG, expected_product: 'Kế hoạch', deadline: deadline(7),
      critical_overdue_days: 3, competent_authority: 'Lãnh đạo Văn phòng', status: 'DANG_THUC_HIEN',
      assigned_to: IDS.cv1, leader_in_charge: IDS.pcvp, created_by: IDS.pcvp, warning_count: 0,
    },
  ]).select('id, title');
  ...
  const t3 = tasks.find((t) => t.title.endsWith('T3')).id;
  ...
  const { data: dirs, error: e2 } = await db.from('task_directives').insert([
    ...
    { task_id: t3, sender_id: IDS.pcvp, content: `${TAG} D3`, is_read: false },
  ]).select('id, task_id');
  ...
  return {
    ...
    d3: dirs.find((d) => d.task_id === t3).id,
    e2: ev[0].id,
    m1: msgs[0].id,
  };
}

-- 0044 — Bộ sẵn "dữ liệu thử" của Dọn dữ liệu (qt_don, 0042/0043) xoá thêm phòng thử E2E (dm_don_vi ma LIKE 'E2E\_%', do scripts/seed-demo.mjs tạo
-- để PCVP demo có phòng phụ trách không đụng phòng thật) và mọi phân công phu_trach_phong vào các phòng đó (kể cả của lãnh đạo thật, nếu có).
-- Phần còn lại của hàm giữ nguyên 0043. Dùng trước go-live (docs/KIEM-THU.md).
...
BEGIN
  ...
  IF v_thu THEN
    ...
    -- 4. Phòng thử E2E (seed-demo) và phân công vào phòng đó — sau khi nhiệm vụ gắn phòng đã xoá ở bước 2
    kq := kq || jsonb_build_object(
      'phu_trach_phong_e2e', "public"."qt_don_bang"('public.phu_trach_phong', 'phong LIKE ''E2E\_%''', "p_thuc_hien", v_nv, v_tks, v_vbs, v_tu, v_den),
      'dm_don_vi_e2e', "public"."qt_don_bang"('public.dm_don_vi', 'ma LIKE ''E2E\_%''', "p_thuc_hien", v_nv, v_tks, v_vbs, v_tu, v_den));
  END IF;

-- 0032 — GĐ19 (CH-16): chỉ đạo Thường trực Tỉnh ủy CHI_DAO_TT. Chỉ A0 gửi; người nhận tự tính = Chánh VP + PCVP phụ trách phòng
-- Owner/theo dõi (pcvp_phu_trach 0026) + lãnh đạo VP phụ trách đơn vị ngoài (0029); hạn phản hồi mặc định 2 ngày làm việc (bỏ T7/CN,
-- kl_cau_hinh); CHO_PHAN_HOI tới khi người nhận phản hồi hoặc chuyển thành chỉ đạo con (tra_loi_cho = luồng TT); A0 đóng luồng TT của
-- mình (người nhận không; Y_KIEN giữ 0030). Tin luồng TT chỉ tới người nhận + A0 (không qua nguoi_lien_quan). Quá hạn → canh_bao_quet
-- gửi tin người nhận chưa phản hồi (muc CHI_DAO_TT, nhắc lại theo canh_bao_nhac_lai_ngay), không leo thang. Quyền ghi allowlist theo vai.
-- 1. chi_dao: loại mới, người nhận, chỉ đạo con. Ràng buộc gốc cũ (PHAN_HOI ⇔ có tra_loi_cho) nới: PHAN_HOI luôn có tra_loi_cho; CHI_DAO_TT không có; loại điều hành có tra_loi_cho chỉ khi hàm gắn vào luồng TT.
...
END;
$$;
-- 3. Người nhận chỉ đạo Thường trực (nội bộ): Chánh VP + PCVP phụ trách phòng Owner/phòng theo dõi (kiêm nhiệm lĩnh vực ưu tiên, như 0026) + lãnh đạo VP phụ trách đơn vị ngoài; bỏ tài khoản hệ thống, không trùng.
...
$$;
-- 4. chi_dao_gui (0030): A0 gửi Y_KIEN hoặc CHI_DAO_TT (người nhận tự tính, hạn mặc định 2 ngày làm việc, A0 sửa được nhưng không lùi quá khứ); A1/A2 không gửi CHI_DAO_TT; người nhận chuyển luồng TT thành chỉ đạo con qua p.tra_loi_cho (luồng TT → DA_PHAN_HOI, A0
--    nhận tin). Thân GIA_HAN/GIAO_LAI giữ nguyên 0026/0030.
...
BEGIN
  ...
  ELSE v_han_ph := nullif("p" ->> 'han_phan_hoi', '')::date; END IF;
...
END;
$$;
-- 5. chi_dao_phan_hoi (0030): gốc của một dòng = chính nó nếu không phải PHAN_HOI (chỉ đạo con vẫn là luồng riêng); luồng CHI_DAO_TT chỉ người nhận phản hồi (A2/A3/A1 ngoài danh sách 42501); tin tới A0 mở luồng + người nhận khác.
...
BEGIN
  IF "public"."me_la_a0"() THEN RAISE EXCEPTION 'Thường trực Tỉnh ủy chỉ ghi ý kiến, không phản hồi chỉ đạo.' USING ERRCODE = '42501'; END IF;
  ...
  IF v_goc."id" IS NULL OR NOT "public"."kl_thay_nhiem_vu"(v_goc."nhiem_vu_id") THEN RAISE EXCEPTION 'Không tìm thấy chỉ đạo trong phạm vi của đồng chí.' USING ERRCODE = '42501'; END IF;
  SELECT * INTO v_nv FROM "public"."nhiem_vu" WHERE "id" = v_goc."nhiem_vu_id";
  IF v_goc."loai" = 'CHI_DAO_TT' THEN
    IF NOT ("auth"."uid"() = ANY (v_goc."nguoi_nhan")) THEN RAISE EXCEPTION 'Chỉ người nhận chỉ đạo Thường trực (Chánh Văn phòng, PCVP phụ trách) mới phản hồi.' USING ERRCODE = '42501'; END IF;
  ...
  IF v_goc."loai" = 'CHI_DAO_TT' THEN
    PERFORM "public"."chi_dao_tt_ghi_vet"(v_goc."nhiem_vu_id", format('Phản hồi chỉ đạo Thường trực · %s: %s', v_nv."ma", left(v_noi_dung, 120)), ARRAY[v_goc."nguoi_gui"] || v_goc."nguoi_nhan");
  ELSE
...
END;
$$;
-- 6. chi_dao_dong (0030): A0 đóng được đúng luồng CHI_DAO_TT do mình mở (Y_KIEN vẫn không); luồng TT không ai khác đóng (kể cả người nhận, quan_tri_kl); chỉ đạo con (có tra_loi_cho) do người gửi đóng như chỉ đạo thường.
...
BEGIN
  ...
  IF "public"."me_la_a0"() THEN
    IF v."id" IS NULL OR v."loai" <> 'CHI_DAO_TT' OR v."nguoi_gui" <> "auth"."uid"() THEN RAISE EXCEPTION 'Thường trực Tỉnh ủy chỉ đóng chỉ đạo Thường trực do chính mình gửi.' USING ERRCODE = '42501'; END IF;
  ELSIF v."id" IS NOT NULL AND v."loai" = 'CHI_DAO_TT' THEN RAISE EXCEPTION 'Chỉ Thường trực Tỉnh ủy mới đóng chỉ đạo Thường trực.' USING ERRCODE = '42501';
  ELSIF v."id" IS NULL OR NOT (v."nguoi_gui" = "auth"."uid"() OR "public"."me_quan_tri_kl"()) THEN RAISE EXCEPTION 'Chỉ người ra chỉ đạo mới đóng được chỉ đạo này.' USING ERRCODE = '42501'; END IF;
...
END;
$$;
-- 7. Quá hạn phản hồi (0029): canh_bao thêm mức CHI_DAO_TT + cột chi_dao_id; canh_bao_quet quét luồng TT CHO_PHAN_HOI có han_phan_hoi < ngày tính → tin hệ thống cho người nhận chưa phản hồi, nhắc lại sau canh_bao_nhac_lai_ngay; không leo thang. Vòng 1 giữ nguyên 0029.
...
BEGIN
  ...
  FOR r IN
    ...
    WHERE nv."dong_luc" IS NULL AND nv."tien_do_ma" <> 'HOAN_THANH' AND (t.tt)."muc_canh_bao" IN ('VANG', 'DO', 'DO_DAC_BIET')
    ORDER BY nv."ma"
  LOOP
    v_quet := v_quet + 1;
...
END;
$$;
-- 8. v_chi_dao_tt (security_invoker → RLS chi_dao/nhiem_vu lọc phạm vi): Dashboard A1 (dòng mình là người nhận) và A0 (dòng mình gửi); chờ phản hồi xếp trước, hạn gần trước. 9. Quyền: hàm nội bộ không cấp cho client; ngay_lam_viec_sau và view cho authenticated.
...
DECLARE f text;
BEGIN
  FOREACH f IN ARRAY ARRAY['chi_dao_tt_nguoi_nhan(public.nhiem_vu)', 'chi_dao_tt_ghi_vet(uuid,text,uuid[])'] LOOP
